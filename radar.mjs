...
// 用法：
//   node radar.mjs pull [--source direct-shops]     拉取并入库最新快照
...
async function cmdDaemon(config, db) {
  ...
  if (interval < 60) {
    console.warn("[daemon] 建议 interval >= 60 秒（避免过于频繁地读取公开目录）。");
  }
...
async function cmdProducts(config, db) {
  const source = arg("--source", "direct-shops");
...
async function main() {
  ...
  try {
    switch (cmd) {
      ...
      default:
        console.log(
          `用法: node radar.mjs <pull|watch|daemon|products|offers|history|alerts|sources|submissions|submission-status|serve> ...\n` +
