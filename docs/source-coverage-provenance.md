...
适配器身份决定来源等级；外部报价携带的 `source_type` 不可信。本轮 `direct-shops` 为 `original_crawl`（原店采集），RelayWatch、CardNav、GoAIHop 均为 `third_party`。没有把原店爬取计为商家主动 API 同步，也没有把转录的官方价计为直取官方来源；当前 `merchant_direct` 为零。
