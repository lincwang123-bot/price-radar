...
[返回项目首页](../README.md) · [在线使用 AIradar](https://airadar.vip/)

> 这里保留仓库现有的开发与运维说明。生产功能会持续更新，使用入口与当前数据规则请以网站为准。

以下为采集、运行、数据维护和部署说明。

按产品查看 AI 订阅的店铺、规格和报价，支持固定登记的原始店铺公开目录采集、SQLite 历史记录与规则化盯盘提醒。
...
- `/weekly`：滚动7日同规格变化，支持按截止日期查看往期。至少跨两个有记录的日期且变化达到1%才展示。分享可在浏览器生成带规格、日期的 PNG。2026-09-13 网站增加复制链接、图片下载、X/TG 编辑入口，周报链接包含截止日期和具体规格组；不会自动发布。
...
已收录店铺可从报价旁或 `/claim-shop?new=1` 申请关联 X。店主取得认领码后，将其放在自己的 X 帖子或简介，并提交店铺归属说明；站长在 `/admin/x-claims` 分别核对账号控制和店铺归属后批准。账号链接展示在该店报价及 `/shop?id=…` 的店铺主页，产品报价页提供综合排序和价格排序。2026-09-13 网站已调整为先看购买条件完整度，再比较同币种价格；X 关联和收录不影响自然排名。关联不代表信誉认证。店主可自愿打开 X 分享店铺页；本站不代发帖子。

更换已有账号需明确确认替换，撤销后立即停止展示，操作留存审核记录。联系邮箱、内部依据及私密进度链接不进入公开页面；不自动提取旧投稿联系方式。详情、核验方式与数据维护见 [店主 X 认领说明](merchant-x-claims.md)。
...
- 补充 `Max5X`、`20×` 等明确写法；API 中转优先于标题中的 Pro / Max 号池词识别。“库存紧张”保留为可购买报价。
- 接码业务已下架：移除导航、商品、搜索、站点地图及旧报价跳转；原店采集与后续入库共用过滤规则。历史数据保留作追溯，邮箱账号与成品账号的“已接码”属性不作为独立接码业务。
...
`/robots.txt` 声明站点地图与私有路径限制，保留边缘既有搜索/训练机器人策略；`/sitemap.xml` 从当前真实产品动态生成，复用前台目录加入有公开报价或官方参考报价的品牌分类页与产品汇总页，同时保留现有按来源区分的产品详情页。新增目录条目仅使用 `family`、`product` 规范参数，不收录空目录、排序/渠道/交付方式筛选组合、投稿、后台或 API。由于行情观察时间不等于页面实际修改时间，暂不输出可选 `lastmod`，避免虚假更新信号。
...
2026-09-06 只读核验生产 `/opt/linc/apps/price-radar/config.json` 不存在，旧版 21 个目标来自内置默认；因此发布新版代码即可采用 23 个默认目标，无需创建或覆盖配置。若其他环境显式设置 `sources.direct-shops.targets`，默认数组不会覆盖自定义选择，应先将完整配置备份至 Git 忽略的 `backups/`（目录 0700、文件 0600），只合并经确认的新目标，保留其余字段和通知秘密，禁止打印完整配置。现有提醒配置应按已登记来源单独核对。
...
- 文章阅读依旧使用原有机器人、管理员、可信代理、请求限额过滤；未知文章与带任意来源的请求不会增加文章归因。搜索词与搜索曝光仍以 Search Console 为准。

### 2026-09-09 业务范围调整

中转站、API 服务及原 API/CDK/额度分类已下架。中央目录策略排除历史报价、旧详情与跳转、站点地图、关注与广告；GoAIHop 中转采集已停用，旧配置无法重新启用。原始历史数据保留，普通订阅与订阅 CDK 交付继续收录。底部赞助入口可咨询云服务器、IP 服务、网络检测、域名与开发者工具合作。
