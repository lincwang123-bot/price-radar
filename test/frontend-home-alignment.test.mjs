...
test('homepage reserves delivery columns without inventing quotes and keeps other delivery links',async()=>{
 ...
 try{
  storeSnapshot(db,{source:'direct-shops',snapshotId:'alignment',fetchedAt:now,products:[
   ...
   {productId:'email-accounts',name:'Gmail 邮箱',currency:'CNY',offers:[offer('gmail','Gmail 邮箱账号',5)]},
  ]});
  ...
  assert.doesNotMatch(row('email-accounts'),/data-delivery-slot/);
  assert.match(row('email-accounts'),/data-directory-minimum/);
