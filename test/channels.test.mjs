...
test('无匹配平台仍保留合法品牌选项，不回退全部或接受未知品牌',async()=>{const db=openDb(':memory:'),app=createApp({db});try{await new Promise(r=>app.listen(0,'127.0.0.1',r));const base='http://127.0.0.1:'+app.address().port;const html=await(await fetch(base+'/?family=claude&channel=ldxp')).text();assert.match(html,/data-family-filter="claude"/);assert.match(html,/暂无可用报价/);assert.doesNotMatch(html,/!groups.length/);const unknown=await(await fetch(base+'/?family=madeup&channel=ldxp')).text();assert.doesNotMatch(unknown,/data-family-filter="madeup"/)}finally{await new Promise(r=>app.close(r));db.close()}});
test("渠道筛选重算当前价、保留翻页和返回品牌参数",async()=>{
 ...
 storeSnapshot(db,{source:"ldxp-goods",snapshotId:"fixture",products:[{productId:"claude-pro-month",name:"Claude Pro",platform:"Claude",lowestPrice:1,currency:"CNY",offers}]});
 await new Promise(r=>app.listen(0,"127.0.0.1",r));const html=await(await fetch(`http://127.0.0.1:${app.address().port}/product?source=ldxp-goods&id=claude-pro-month&channel=16688&page=2`)).text();
