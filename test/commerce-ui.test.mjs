...
test('广告无默认样例，approved匹配活动独立展示且不改变自然顺序',async()=>fixture(async({base,analytics})=>{
 ...
 const ad=await(await fetch(base+'/advertise')).text();assert.match(ad,/人工确认/);assert.match(ad,/首期试行价/);assert.doesNotMatch(ad,/锁定早期价格/);assert.match(ad,/rel="canonical" href="https:\/\/airadar.vip\/advertise"/);assert.match(await(await fetch(base+'/sitemap.xml')).text(),/\/advertise<\/loc>/);
