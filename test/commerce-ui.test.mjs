...
test('广告无默认样例，approved匹配活动独立展示且不改变自然顺序',async()=>fixture(async({base,analytics})=>{
 ...
 const after=await(await fetch(base+route)).text();assert.match(after,/Sponsored \/ 广告/);assert.match(after,/rel="noopener noreferrer nofollow sponsored"/);assert.equal(after.match(/<tbody>([\s\S]*?)<\/tbody>/)[1],before.match(/<tbody>([\s\S]*?)<\/tbody>/)[1]);assert.match(after,/data-outbound-target="https:\/\/merchant.test/);assert.match(after,/href="\/go\?/);assert.match(after,/link.dataset.outboundTarget/);assert.match(after,/searchParams.set\('ack','1'\)/);assert.match(after,/topic=merchant_claim/);
 const card=after.match(/<article class="sponsor-card">[\s\S]*?<\/article>/)[0];assert.match(card,/¥110/);assert.match(card,/在售/);assert.match(card,/原店采集/);assert.match(card,/datetime="/);assert.doesNotMatch(card,/商家同步|认证|排名|#2/);
