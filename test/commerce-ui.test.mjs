...
test('共享域旧merchant_id不能冒认独立商家或产生广告曝光',()=>{
 for(const host of ['16688.com.cn','wzyp.cn']){
  const product={source:'ldxp-goods',product_id:'chatgpt-plus-recharge',snapshot_id:'old'};
...
test('广告无默认样例，approved匹配活动独立展示且不改变自然顺序',async()=>fixture(async({base,analytics})=>{
 ...
 const after=await(await fetch(base+route)).text();assert.match(after,/Sponsored \/ 广告/);assert.match(after,/rel="noopener noreferrer nofollow sponsored"/);assert.equal(after.match(/<tbody>([\s\S]*?)<\/tbody>/)[1],before.match(/<tbody>([\s\S]*?)<\/tbody>/)[1]);assert.match(after,/data-outbound-target="https:\/\/merchant.test/);assert.match(after,/href="\/go\?/);assert.match(after,/link.dataset.outboundTarget/);assert.match(after,/searchParams.set\('ack','1'\)/);assert.match(after,/href="\/claim-shop\?shop=[a-f0-9]{24}/);
 ...
 const privacy=await(await fetch(base+'/privacy')).text();assert.match(privacy,/更新于2026年9月9日/);assert.match(privacy,/不代表屏幕可见曝光或真实人数/);
 const ad=await(await fetch(base+'/advertise')).text();assert.match(ad,/人工确认/);assert.match(ad,/获取报价/);assert.doesNotMatch(ad,/首期试行价|价目版本/);assert.doesNotMatch(ad,/锁定早期价格/);assert.match(ad,/rel="canonical" href="https:\/\/airadar.vip\/advertise"/);assert.match(await(await fetch(base+'/sitemap.xml')).text(),/\/advertise<\/loc>/);
 for(const path of ['/','/advertise','/sources',route]){const h=await(await fetch(base+path)).text();const visible=h.replace(/<script[\s\S]*?<\/script>|<style[\s\S]*?<\/style>/g,'').replace(/<[^>]+>/g,'');assert.doesNotMatch(visible,/补充源|CardNav|GoAIHop|LDXP|RelayWatch/);assert.doesNotMatch(visible,/已认证商家|保障购买|保证销量/);}
}));
...
test('跨店新入口保留被自然去重来源的已批准广告，单次GET曝光且不改变自然排序',async()=>fixture(async({base,db,analytics})=>{
 storeSnapshot(db,{source:'ldxp-goods',snapshotId:'ad-duplicate',products:[{productId:'chatgpt-plus-recharge',name:'ChatGPT Plus',platform:'ChatGPT',currency:'CNY',offers:[{offerId:'ad-copy',title:'ChatGPT Plus 代充 1个月',storeName:'虚构店铺1',price:110,currency:'CNY',status:'in_stock',stockCount:1,url:'https://merchant.test/product/1'}]}]});
 const route='/?family=chatgpt&product=chatgpt-plus',before=await(await fetch(base+route)).text();
 analytics.outbound.saveCampaign({id:'cross-source',merchant_id:'domain:merchant.test',source:'ldxp-goods',product_id:'chatgpt-plus-recharge',offer_id:'ad-copy',label:'跨来源已批准广告',placement:'sponsored_product',start_at:new Date(Date.now()-1000).toISOString(),end_at:new Date(Date.now()+3600000).toISOString()},{approve:true});
 ...
 const link=after.match(/<article class="sponsor-card"[^>]*>[\s\S]*?href="([^\"]+)"/)[1].replaceAll('&amp;','&');assert.equal(new URL(link,base).searchParams.get('source'),'ldxp-goods');assert.equal((await fetch(base+link,{redirect:'manual'})).status,200);
}));
