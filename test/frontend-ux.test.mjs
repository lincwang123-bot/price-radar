...
test('详情历史走势遵守独立站框架筛选',async()=>{
 ...
 try{
  ...
  await new Promise(r=>app.listen(0,'127.0.0.1',r));const base='http://127.0.0.1:'+app.address().port;
  const h=await(await fetch(base+'/product?source=ldxp-goods&id=claude-pro-month&framework=dujiao')).text();assert.match(h,/最低价走势：[^\"]*¥100[^\"]*¥101/);assert.match(h,/共 1 条公开报价/);assert.match(h,/price-display">¥101</);
 }finally{if(app.listening)await new Promise(r=>app.close(r));db.close();}
});
test('公开表单安全退化、方法说明及SSR组合筛选', async()=>{
 ...
 try {
  ...
  const methods=await(await fetch(base+'/sources')).text();assert.match(methods,/价格与库存/);assert.doesNotMatch(methods,/快照|产品记录|报价记录|补充源/);
...
test('提醒新到旧分页，保留当时价格',async()=>{
 ...
 try{
  ...
  for(let i=0;i<32;i++)insert.run(new Date(Date.UTC(2026,8,1,0,i)).toISOString(),'ldxp-goods','claude-pro-month','Claude Pro','drop_pct','历史价格 ¥'+i);
  await new Promise(r=>app.listen(0,'127.0.0.1',r));const base='http://127.0.0.1:'+app.address().port;
...
test('详情规格与筛选贯穿翻页，无匹配渠道不泄漏全部报价',async()=>{
 const db=openDb(':memory:'),app=createApp({db});
 try{
  storeSnapshot(db,{source:'ldxp-goods',snapshotId:'state',products:[{productId:'claude-pro-month',name:'Claude Pro',platform:'Claude',currency:'CNY',offers:Array.from({length:12},(_,i)=>({offerId:String(i),title:'Claude Pro 代充 1个月',price:100+i,status:'in_stock',stockCount:1,url:'https://16688.com.cn/goods/'+i}))}]});
  await new Promise(r=>app.listen(0,'127.0.0.1',r));const base='http://127.0.0.1:'+app.address().port;
  ...
  const empty=await(await fetch(base+'/product?source=ldxp-goods&id=claude-pro-month&channel=ldxp')).text();assert.match(empty,/共 0 条公开报价/);assert.doesNotMatch(empty,/data-store-risk[^>]*href="https:\/\/16688/);
 }finally{if(app.listening)await new Promise(r=>app.close(r));db.close();}
});
test('首页直接列产品，一步展示全部规格店铺，旧详情仍可切换规格',async()=>{
 ...
 try{
  ...
  const detail=await(await fetch(base+'/product?source=ldxp-goods&id=chatgpt-plus-recharge')).text();assert.match(detail,/aria-label="选择规格"/);assert.match(detail,/price-display">¥100</);assert.match(detail,/共 1 条公开报价/);
