...
test('指南统计仅记录固定文章标识和同站下一步比价，不保存来源或查询参数',()=>{
 ...
 try{
  ...
  const r=a.contentReport(7,now),row=r.rows.find(r=>r.path===article);assert.equal(row.views,1);assert.equal(row.quoteVisits,1);assert.equal(r.rows.find(r=>r.path==='/help/price-alerts').views,1);assert.equal(r.rows.length,11);assert.ok(r.rows.some(r=>r.path==='/guides/chatgpt-go-vs-plus'));
