...
import { ADMIN_COPY_SCRIPT, ADMIN_COPY_HASH } from '../lib/admin-copy.mjs';
...
test("后台默认拒绝，认证/过期/退出/CSRF/审计/转义完整边界",async()=>{
  ...
  app=createApp({db,submissionsDb,analytics,adminOptions:{username:"owner",passwordHash,origin,now:()=>time}});await new Promise(r=>app.listen(0,"127.0.0.1",r));
  ...
  }finally{await new Promise(r=>app.close(r));db.close();submissionsDb.close();analytics.close();}
});
