...
test('same SKU controlled description excludes simplified/traditional no-after-sales in object and stored JSON',()=>{
  for(const description of ['质保首登。※速刷商品 不提供售後，請注意!pro 5x过期不影響使用','无售后','沒有質保','售後概不負責']){
    for(const stringify of [false,true]){
      ...
      for(const source of ['direct-shops','ldxp-goods'])assert.equal(publicOfferAllowed(source,row),false);
    }
  }
});
...
test('existing stored snapshots are filtered for current, history and alert group projection without deleting rows',()=>{
  ...
  try{
    for(const snapshotId of ['old','new'])storeSnapshot(db,{source:'ldxp-goods',snapshotId,fetchedAt:new Date().toISOString(),products:[{productId:'claude-pro-month',name:'Claude Pro',currency:'CNY',offers:[
    ...
    const current=projectProduct(db,'ldxp-goods',snapshot,product);
    ...
    const history=quoteSeries(db,{source:'ldxp-goods',productId:'claude-pro-month'});
