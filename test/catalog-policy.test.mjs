...
test('retired service titles cannot hide behind subscription or email categories', () => {
  for (const title of services) {
    ...
    for (const source of ['direct-shops','ldxp-goods','cardnav-official']) assert.equal(publicOfferAllowed(source,{title}), false, source+title);
  }
...
test('legacy snapshots cannot expose retired products through pages, search, sitemap, market or outbound links', async () => {
  ...
  try {
    ...
    for(const source of ['direct-shops','ldxp-goods']) storeSnapshot(db,{source,snapshotId:'legacy',products:[
    ...
    for(const source of ['direct-shops','ldxp-goods']) for(const [product,offerId] of [['old-custom-id','custom'],['openai-phone-verification','old'],['verification-service','old2'],['chatgpt-plus','hidden'],['email-accounts','gmail-code']]) {
...
test('API and relay retirement catches mixed subscription titles and source-only legacy records', () => {
  for(const title of ['API Cursor Pro 2600积分','Claude-Kiro API KEY 100M Token','100刀Codex API中转额度(纯Pro号池)','AI平台直充100美元额度-Claude Max / 官API','Claude Pro 网页镜像','GPT Pro 中转余额','api Gplus月订阅','Claude 10刀余额充值']){
    assert.equal(retiredCatalogItem({title}),true,title);
    assert.equal(classifyDirectOffer({title}),null,title);
    assert.equal(publicOfferAllowed('ldxp-goods',{title},{product_id:'claude-pro'}),false,title);
  }
...
test('retired relay catalog is absent from legacy routes, SEO, sponsors and retention while subscriptions survive', async () => {
 ...
 try{
  ...
  for(const source of ['ldxp-goods','direct-shops','ldxp-goods'])storeSnapshot(db,{source,snapshotId:'legacy-api',products:[
  ...
  for(const source of ['ldxp-goods','direct-shops','ldxp-goods','goaihop-relay']){
  ...
  const kept=await fetch(base+'/go?source=ldxp-goods&snapshot=legacy-api&product=claude-pro&offer=kept&ack=1',{redirect:'manual'});assert.equal(kept.status,302);
  const hidden=await fetch(base+'/go?source=ldxp-goods&snapshot=legacy-api&product=claude-pro&offer=api&ack=1',{redirect:'manual'});assert.equal(hidden.status,404);
