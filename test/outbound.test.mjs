...
const offer={source:'direct-shops',snapshot_id:'s1',product_id:'p',offer_id:'o'};
test('existing analytics online backup includes campaigns and outbound tables without a fourth database',async()=>{
 ...
 try{
  a.outbound.saveCampaign({id:'fixture',merchant_id:'domain:shop.example',source:'direct-shops',product_id:'p',offer_id:'o',label:'fixture only',placement:'sponsored_product',start_at:'2026-09-01',end_at:'2026-10-01'});
...
function refresh(ctx,changes={}){storeSnapshot(ctx.db,{source:'direct-shops',snapshotId:'s2',fetchedAt:date.toISOString(),products:[{productId:'p',name:'fixture',offers:[{offerId:'o',sourceId:'merchant',price:12,currency:'CNY',status:'in_stock',url:'https://shop.example/buy',...changes}]}]});}
test('links from an open page survive a new snapshot and resolve the latest quote',()=>{
 const ctx=fixture();try{
  ...
  refresh(ctx);
  const current=resolveOutboundOffer(ctx.db,{source:'direct-shops',snapshot:'s1',product:'p',offer:'o'},date.getTime());
...
test('shared platform old links retain the same shop product and campaign checks use the latest quote',()=>{
 ...
 const sponsored=fixture();try{
  const campaign={id:'refresh-ad',merchant_id:'domain:shop.example',source:'direct-shops',product_id:'p',offer_id:'o',label:'fixture',placement:'sponsored_product',start_at:'2026-09-01',end_at:'2026-10-01'};
...
test('outbound daily identities rotate and per-minute rates are bounded',()=>{
 const ctx=fixture();try{
  const row={source:'direct-shops',product_id:'p',offer_id:'o',merchant_id:'domain:shop.example'};
...
test('shared-platform shops redirect with separate quote keys but cannot sponsor or open aggregator targets',()=>{
 const ctx=fixture();try{
  ...
  ctx.db.prepare("INSERT INTO offers(source,snapshot_id,product_id,offer_id,price,currency,status,url,merchant_id) VALUES('direct-shops','s1','p','o2',12,'CNY','in_stock','https://16688.com.cn/shop/b','domain:16688.com.cn')").run();
  ...
  assert.equal(ctx.analytics.outbound.campaignsFor({source:'direct-shops',productId:'p'},date).length,0);
  assert.equal(follow(ctx,outboundHref(offer,{}, {placement:'sponsored_product',campaignId:'shared'})).statusCode,404);
  for(const url of ['https://relaywatch.online/item','https://goaihop.com/item','https://cardnav.xyz/item']){ctx.db.prepare('UPDATE offers SET url=? WHERE offer_id=?').run(url,'o');assert.equal(follow(ctx).statusCode,404);}
 }finally{ctx.analytics.close();ctx.db.close();}
});
...
test('Sponsor requires explicit review, relevance, dates and server-derived identity; metrics failure still redirects',()=>{
 const ctx=fixture(),c={id:'c',merchant_id:'domain:shop.example',source:'direct-shops',product_id:'p',offer_id:'o',label:'<script>fixture</script>',placement:'sponsored_product',start_at:'2026-09-01T00:00:00Z',end_at:'2026-10-01T00:00:00Z'};
 try{
  ctx.analytics.outbound.saveCampaign(c,{now:date});assert.equal(ctx.analytics.outbound.campaignsFor({source:'direct-shops',productId:'p'},date).length,0);
  const href=outboundHref(offer,{}, {placement:'sponsored_product',campaignId:'c'});assert.equal(follow(ctx,href).statusCode,404);
  ctx.analytics.outbound.saveCampaign(c,{approve:true,now:date});assert.equal(ctx.analytics.outbound.campaignsFor({source:'direct-shops',productId:'wrong'},date).length,0);
