...
test('共享平台根域不能作为商家，未归属报价键稳定且按报价隔离',()=>{
 for(const host of ['16688.com.cn','www.16688.com.cn','wzyp.cn','www.wzyp.cn','16688.com.cn.','www.16688.com.cn.','wzyp.cn.','relaywatch.online.','www.relaywatch.online.'])assert.equal(merchantIdForUrl(`https://${host}/item/1`),null);
 assert.equal(merchantIdForUrl('https://store.wzyp.cn/'),'domain:store.wzyp.cn');
 const row={source:'ldxp-goods',product_id:'p',offer_id:'a',url:'https://16688.com.cn/item/1',merchant_id:'domain:16688.com.cn'};
 ...
 assert.equal(offerProvenance('ldxp-goods',row,{fetchedAt:at}).merchant_id,null);
});
...
test('来源等级由适配器决定，第三方载荷不能伪造商家直连或官方',()=>{
 for(const source of ['ldxp-goods','cardnav-official','goaihop-relay','ldxp-goods'])assert.equal(offerProvenance(source,{source_type:'merchant_direct'},{fetchedAt:at}).source_type,'third_party');
 assert.equal(offerProvenance('direct-shops',{source_type:'merchant_direct'},{fetchedAt:at}).source_type,'original_crawl');
 assert.equal(quoteSourceLabel({source:'ldxp-goods',source_type:'merchant_direct'}),'第三方采集');
...
test('旧报价可推导中性标签/时间，陈旧文案不声称购买认证',()=>{
 const value=quoteTimeInfo({source:'ldxp-goods',captured_at:at,quote_stale:true},{now:Date.parse(at)+3600000});
