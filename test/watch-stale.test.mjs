...
test("stale 快照不求值、不推进 watermark，恢复后跳过 stale 与上次健康快照比较", () => {
  ...
  const config = {
    rules: [{ id: "fixture-cheapest", kind: "cheapest_changed", source: "direct-shops", product: "fixture-product" }],
  };
  try {
    storeSnapshot(db, snapshot("healthy-1", "2026-09-05T00:00:00.000Z"));
    assert.deepEqual(runWatch(db, config), []);
    assert.equal(metaGet(db, "watch_wm_direct-shops"), "healthy-1");
    ...
    assert.deepEqual(runWatch(db, config), []);
    assert.equal(metaGet(db, "watch_wm_direct-shops"), "healthy-2");
  } finally {
    db.close();
  }
});
