...
test('explicit classified G Plus and Codex offers survive the bare-tier fallback guard', () => {
  ...
  for (const source of ['direct-shops', 'ldxp-goods']) for (const [id, key, title] of cases) {
...
test('classified alias titles still reject mixed subscription bundles and untrusted search fallback', () => {
  ...
  for (const source of ['direct-shops', 'ldxp-goods']) {
...
test('16688 exact product URLs deduplicate changed titles/specs across sources, preferring direct', () => {
  const entries = [
    { list: { source: 'ldxp-goods' }, product: { currency: 'CNY' }, offer: quote('Claude Pro 旧标题', { url: 'https://16688.com.cn/goods/G69013943?utm_source=legacy', comparison_known: false }) },
...
test('Plus folds into one product but keeps source, delivery, currency and duration variants', () => {
  const lists = [list('direct-shops', 'chatgpt-plus-recharge', [quote('ChatGPT Plus 月卡代充'), quote('ChatGPT Plus 月卡代充', { price: 80, offer_id: 'cheaper' }), quote('ChatGPT Plus 年卡代充', { comparison_key: 'year:recharge:CNY' })]), list('ldxp-goods', 'chatgpt-plus', [quote('ChatGPT Plus 月卡成品号', { comparison_key: 'month:account:CNY' })]), list('ldxp-goods', 'search-plus', [quote('ChatGPT Plus 月卡代充')])];
...
test('LDXP uses actual title and skips generic search matches and ordinary accounts', () => {
  ...
  assert.equal(category([list('direct-shops', 'chatgpt-plus', [quote('普通账号')])], 'chatgpt').products[0].variants.length, 0);
});
...
test('only fresh available positive known-currency known-spec quotes enter directory', () => {
  ...
  const product = category([list('ldxp-goods', 'claude-pro-month', offers)], 'claude').products[0];
  assert.deepEqual(product.entries.map(entry => entry.offer.offer_id), ['valid']);
});

test('trusted direct IDs support missing titles but search IDs never do', () => {
  const lists = ['direct-shops', 'ldxp-goods'].map(source => list(source, 'claude-pro-month', [quote(null)]));
  assert.equal(category(lists, 'claude').products[0].entries.length, 1);
});
...
test('ordinary accounts keep their own identity and unknown specs retain safe product references', () => {
  const lists = [list('direct-shops', 'chatgpt-free-account', [quote('普通账号', { comparison_known: false })]), list('direct-shops', 'education-email', [])];
...
test('trusted canonical product keeps reference when every offer title lacks brand evidence', () => {
  const products = category([list('direct-shops', 'chatgpt-plus-recharge', [quote('【自营】Plus 已接码')])], 'chatgpt').products;
  assert.equal(products.length, 1);
...
test('complete directory quotes combine all sources and retain unknown specifications', () => {
  const lists = [
    ...
    list('direct-shops', 'chatgpt-plus', [quote('【自营】Plus 已接码', { offer_id: 'p1', url: 'https://c.example/goods/1', comparison_known: false, comparison_key: 'mixed' })]),
...
test('deduplication retains different shops and products while preferring original source', () => {
  ...
  const product = category([
    list('ldxp-goods', 'chatgpt-plus', [quote('ChatGPT Plus 月卡', { url: 'https://shop.example/goods/1?utm_source=mirror', offer_id: 'mirror', price: 99 }), quote('ChatGPT Plus 月卡', { url: 'https://shop.example/goods/2', offer_id: 'other-product', price: 120 })]),
...
test('complete quotes exclude unsafe destinations, no warranty, stale and unavailable offers', () => {
  ...
  const product = category([list('ldxp-goods', 'chatgpt-plus', [...invalid.map(patch => quote('ChatGPT Plus 月卡', patch)), quote('ChatGPT Plus 月卡', { offer_id: 'valid', comparison_known: false })])], 'chatgpt').products[0];
...
test('trusted fallback never assigns an explicit different brand to Plus', () => {
  const product = category([list('direct-shops', 'chatgpt-plus', [quote('Gemini Plus 月卡'), quote('Claude Pro 月卡'), quote('Plus 月卡', { offer_id: 'bare-plus' })])], 'chatgpt').products[0];
...
test('unverified shop names never affect shared-platform deduplication or merchant counts', () => {
  const product = category([
    { ...list('ldxp-goods', 'chatgpt-plus', [quote('ChatGPT Plus 月卡', { offer_id: 'old', store_name: '名字一' })]), fetchedAt: '2026-09-01T00:00:00Z' },
...
test('mixed subscription brands and unknown-brand tier fallbacks are excluded', () => {
  const directory = buildProductDirectory([list('ldxp-goods', 'chatgpt-plus', [
...
test('trusted bare Plus retains actual CDK and app delivery terms without allowing foreign brands', () => {
  ...
  const product = category([list('direct-shops', 'chatgpt-plus', [
