...
export function createAdmin({submissionsDb,db,backupDir,analytics,merchantBridgeDir,username=process.env.ADMIN_USERNAME,passwordHash=process.env.ADMIN_PASSWORD_HASH,origin=process.env.PUBLIC_ORIGIN,now=()=>Date.now()}) {
  ...
  return async function admin(req,res,url) {
    ...
    try {
      ...
      if(req.method==="POST") {
        ...
        if (merchantMatch) {
          reviewMerchantApplication(submissionsDb, merchantMatch[1], {
          ...
          }, { bridgeDir: merchantBridgeDir, now: new Date(now()) });
      ...
      if (merchantMatch) {
        ...
        html(res, 200, '店铺审核详情', merchantManifestWarning(byIdentity) + merchantReviewContent({ application, actions: application.actions, csrfToken: s.csrf, health }) + logout); return true;
      }
