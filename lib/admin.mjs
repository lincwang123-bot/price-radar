...
import { merchantInboxContent, merchantReviewContent } from './merchant-ui.mjs';
    ...
    try {
      ...
      if(req.method==="POST") {
        ...
        if (conversionMatch) {
          ...
          const values = { shopName:f.shopName, shopUrl:f.shopUrl, contact:f.contact, details:f.details, note:f.note,
        ...
        if (merchantMatch) {
          ...
          if (review.action === 'test') queueMerchantPreflight(submissionsDb, merchantMatch[1], review, preflightOptions);
          else {
            ...
            if (['approve', 'reject', 'pause'].includes(review.action)) syncPreflightManifest(submissionsDb, { ...preflightOptions, excludeApplicationId: merchantMatch[1] });
      ...
      if (conversionMatch) {
        const submission = supplySubmissionForConversion(submissionsDb, conversionMatch[1]);
        const values = conversionFormValues({ shopName:'', shopUrl:submission.source_url, contact:submission.contact,
      ...
      if (url.pathname === '/admin/merchants') {
        ...
        html(res, 200, '店铺审核', merchantManifestWarning(byIdentity) + merchantInboxContent({ ...result, status, counts, health }) + logout); return true;
      }
      const merchantMatch = /^\/admin\/merchants\/(MA-[A-Z0-9-]+)$/.exec(url.pathname);
      if (merchantMatch) {
        ...
        html(res, 200, '店铺审核详情', merchantManifestWarning(byIdentity) + merchantReviewContent({ application, actions: application.actions, csrfToken: s.csrf, health, preflight, preflightCanApprove: !!preflight?.canApprove }) + logout); return true;
      }
      ...
      const m=/^\/admin\/submission\/(FB-[A-Z0-9-]+|CO-[A-Z0-9-]+)$/.exec(url.pathname);
      if(m){const isFb=m[1].startsWith("FB-");const row=(isFb?submissionsDb.prepare("SELECT * FROM feedback_submissions WHERE public_id=?"):submissionsDb.prepare("SELECT * FROM cooperation_submissions WHERE public_id=?")).get(m[1]);if(!row)throw new SubmissionError(404,"记录不存在");
      ...
      const isFeedback=kind==="feedback";const clauses=[],args=[];
      if(kind!=="feedback"){clauses.push("topic=?");args.push(kind);}if(status){clauses.push("status=?");args.push(status);}
      const where=clauses.length?" WHERE "+clauses.join(" AND "):"";
      const total=Number((isFeedback?submissionsDb.prepare("SELECT COUNT(*) n FROM feedback_submissions"+where):submissionsDb.prepare("SELECT COUNT(*) n FROM cooperation_submissions"+where)).get(...args).n);
      const rows=(isFeedback?submissionsDb.prepare("SELECT public_id,created_at,subject,status FROM feedback_submissions"+where+" ORDER BY id DESC LIMIT 25 OFFSET ?"):submissionsDb.prepare("SELECT public_id,created_at,subject,status FROM cooperation_submissions"+where+" ORDER BY id DESC LIMIT 25 OFFSET ?")).all(...args,(n-1)*25);
