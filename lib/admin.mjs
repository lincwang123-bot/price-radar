...
function html(res,status,title,body) { res.statusCode=status;res.setHeader("Content-Type","text/html; charset=utf-8");res.end(page(title,body).replace('<a href="/admin/health">','<a href="/admin/analytics">访问统计</a><a href="/admin/health">')); }
...
export function createAdmin({submissionsDb,db,backupDir,analytics,username=process.env.ADMIN_USERNAME,passwordHash=process.env.ADMIN_PASSWORD_HASH,origin=process.env.PUBLIC_ORIGIN,now=()=>Date.now()}) {
  ...
  return async function admin(req,res,url) {
    ...
    try {
      ...
      if(url.pathname==="/admin/analytics"){html(res,200,"访问统计",analyticsContent(analytics,Number(url.searchParams.get("days")))+logout);return true;}
      ...
      if(url.pathname==="/admin/health") {
        ...
        const backups=[['投稿',backupDir],['访问统计',process.env.ANALYTICS_BACKUP_DIR]].map(([name,dir])=>{const b=dir?backupStatus(dir):{ok:false};return `<article><h3>${name}</h3><p>${b.ok?"已通过隔离恢复验证":"暂无有效备份或备份已过期"}</p><p>最近备份：${escape(displayTime(b.createdAt))} · 大小：${b.bytes?Math.ceil(b.bytes/1024)+" KB":"—"}</p></article>`;}).join("");
        html(res,200,"采集与备份",`<section><h2>行情快照</h2><div class="table"><table><thead><tr><th>数据来源</th><th>最后快照（北京时间）</th><th>累计快照</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${escape(r.source)}</td><td>${escape(displayTime(r.last_snapshot))}</td><td>${Number(r.snapshots)}</td></tr>`).join("")}</tbody></table></div><h2>最近采集状态</h2>${health.map(h=>`<article><strong>${escape(h.source)}</strong> · ${escape(label(h.status||""))}<p>${escape(displayTime(h.checkedAt))}</p>${h.targets?`<ul>${h.targets.map(t=>`<li>${escape(t.name)}：${escape(label(t.status))} · 最近成功 ${escape(displayTime(t.lastSuccess))}</li>`).join("")}</ul>`:""}</article>`).join("")||"尚无采集状态记录"}</section><section><h2>本机备份</h2>${backups}<p>本机备份不能防止整台主机丢失；尚未配置异地副本。</p></section>${logout}`);return true;
      }
      const m=/^\/admin\/submission\/(FB-[A-Z0-9-]+|CO-[A-Z0-9-]+)$/.exec(url.pathname);
