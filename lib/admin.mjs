...
import { merchantMailStatus, mailConfiguration } from './merchant-mail.mjs';
import { supplyIntakeReview } from './merchant-workflow-ui.mjs';
  ...
  return async function admin(req,res,url) {
    ...
    try {
      ...
      if(req.method==="POST") {
        ...
        if (merchantMatch) {
          const review = {
            action: f.action, note: f.note, actor: username, expectedVersion: Number(f.version),
            ownershipConfirmed: f.ownershipConfirmed === 'true', permissionConfirmed: f.permissionConfirmed === 'true',
