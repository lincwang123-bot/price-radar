...
const oldPrice = node => node.tag === 'del' || node.tag === 's'
  || /(?:old|original|compare|market|list)[-_ ]?price|price[-_ ]?(?:old|original|compare|strike)|(?:^|\s)(?:line-through|strikethrough)(?:\s|$)/i.test(node.attrs.class || '')
  || /text-decoration(?:-line)?\s*:[^;]*\bline-through\b/i.test(node.attrs.style || '');
const currentPriceText = node => oldPrice(node) ? '' : htmlText(node.parts.map(p => typeof p === 'string' ? p : currentPriceText(p)).join(' '));
function visiblePrices(node) {
  // Remove obsolete-price subtrees before reading a containing price block;
  // filtering just the crossed-out node leaves its amount in the parent text.
  const obsolete = n => {for(let p=n;p;p=p.parent)if(oldPrice(p))return true;return false;};
  const candidates = descendants(node).filter(n => !obsolete(n) && !['meta', 'link'].includes(n.tag)
    && (/price|价格|售价/.test(n.attrs.class || '') || ['strong', 'b'].includes(n.tag) || n.attrs.itemprop === 'price'));
  const prices = candidates.flatMap(n => {
    const t = currentPriceText(n);
...
export function discoverCatalog(html, origin) {
  ...
  const moduleUrls=doc.all.filter(n=>n.tag==='link'&&n.attrs.rel==='modulepreload').map(n=>publicPageUrl(n.attrs.href,origin,'script')).filter(Boolean);
  return { entries: [...entries.values()], catalogLinks: [...catalogLinks], scriptUrls: [...new Set(scriptUrls)], moduleUrls:[...new Set([...moduleUrls,...scriptUrls])] };
}
