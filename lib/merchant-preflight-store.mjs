...
import { isPreflightRunning } from './merchant-preflight-lock.mjs';
...
export function latestMerchantPreflight(db, id, { resultsDir, now = new Date() } = {}) {
  ...
  const reply=latestMerchantReply(db,id);
  // The result must match the current application (above) and postdate the reply.
  // Restoring a queue item changes its version; conversion also creates a new version sequence.
  const matchesReply=!reply||Date.parse(request.requestedAt)>=Date.parse(reply.createdAt);
  return { ...request, result, fresh, canApprove: !!(!application?.isSupplyIntake && matches && matchesReply && fresh && result?.status === 'ready' && result.validCount > 0),
    status: !matches || (result && !fresh) ? 'expired' : result?.status || (invalid ? 'invalid' : Date.parse(request.expiresAt) <= timestamp ? 'expired' : isPreflightRunning(resultsDir, request.id, timestamp) ? 'running' : 'pending') };
}
...
function pendingRequests(db, resultsDir, now) {
  return db.prepare(`SELECT p.* FROM merchant_preflight_requests p WHERE p.requested_at<=? AND p.expires_at>?
    ...
    .filter(request => { const app=testSubject(db,request.applicationId);
      if(!app||app.status!=='pending'||app.version!==request.applicationVersion||app.identity!==request.identity||app.linkedApplication||app.existingApplication||app.urlError)return false;
...
export function queueAutomaticPreflight(db,id,{bridgeDir=merchantBridgeDir(db),resultsDir,now=new Date(),force=false}={}) {
  ...
  // A manual click promotes an existing waiting/running request instead of
  // revoking it and starting a duplicate while another worker is reading it.
  if (force && prior && prior.application_version === app.version && prior.identity === app.identity) {
    const current = latestMerchantPreflight(db, id, { resultsDir, now });
    if (['pending', 'running'].includes(current?.status)) {
      syncPreflightManifest(db, { bridgeDir, resultsDir, now });
      return current;
    }
  }
  if(prior&&prior.application_version===app.version&&prior.identity===app.identity&&!force) {
...
export function reconcileAutomaticPreflights(db,options={}) {
  ...
  for(const app of [...apps,...listSupplyIntakes(db)]) {
    if(app.actions?.at(-1)?.action==='request_info'){
      const reply=latestMerchantReply(db,app.id);
      if(!reply||reply.applicationVersion!==app.version||Date.parse(reply.createdAt)<=Date.parse(app.actions.at(-1).createdAt))continue;
    }
...
export function queueMerchantPreflight(db, id, review, { bridgeDir = merchantBridgeDir(db), resultsDir, now = new Date() } = {}) {
  ...
  try {
    ...
    if (application.status !== 'pending') fail(422, '请先恢复为待审核，再测试接入');
