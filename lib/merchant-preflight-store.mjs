...
export function reconcileAutomaticPreflights(db,options={}) {
  ...
  for(const app of [...apps,...listSupplyIntakes(db)]) {
    ...
    if(preflight?.result)queueMerchantMail(db,{id:app.id,email:app.email,stage:preflight.result.status==='ready'?'ready':'need_info',eventKey:app.id+':result:'+preflight.id,now:options.now});
  }
