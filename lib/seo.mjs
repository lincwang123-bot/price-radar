...
export function seoRoute(req,res,url,db,loadDirectory=()=>[]){
 ...
 if(url.pathname.startsWith('/api/')||['/submit','/submit-shop','/following','/unsubscribe','/claim-shop'].includes(url.pathname))res.setHeader('X-Robots-Tag','noindex, nofollow');
...
export function decorateSeo(html,url,db,status=200){
 const privatePage=['/submit','/submit-shop','/following','/unsubscribe','/claim-shop'].includes(url.pathname)||status>=400;
 ...
 if(url.pathname==='/weekly'){
  ...
  title='AI 产品行情周报'+(date?' · 截至 '+date:'')+' · AirRadar';
