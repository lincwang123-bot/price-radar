...
import {officialPageData,officialProductHref,OFFICIAL_PATH,OFFICIAL_SOURCE} from './official-prices.mjs';
...
const SITE_VERIFICATION_PATH='/3135103.txt';
const SITE_VERIFICATION_BODY=readFileSync(new URL('../3135103.txt',import.meta.url));
...
export function sitemap(db,directory=[]){const urls=[SITE_ORIGIN+'/',SITE_ORIGIN+'/sources',SITE_ORIGIN+'/privacy',SITE_ORIGIN+'/advertise',SITE_ORIGIN+'/weekly',SITE_ORIGIN+OFFICIAL_PATH];
...
export function seoRoute(req,res,url,db,loadDirectory=()=>[]){
 ...
 if(url.pathname===SITE_VERIFICATION_PATH){
  ...
  res.writeHead(200,{'Content-Type':'text/plain; charset=utf-8','Content-Length':SITE_VERIFICATION_BODY.length,'X-Robots-Tag':'noindex, nofollow'});
  res.end(req.method==='HEAD'?undefined:SITE_VERIFICATION_BODY);return true;
 }
 ...
 if(['/index.html','/product/','/sources/','/privacy/','/alerts/','/advertise/','/official-prices/'].includes(url.pathname)&&['GET','HEAD'].includes(req.method)){res.writeHead(301,{Location:(url.pathname==='/index.html'?'/':url.pathname.slice(0,-1))+url.search});res.end();return true;}
...
export function decorateSeo(html,url,db,status=200){
 ...
 const official=url.pathname===OFFICIAL_PATH||product?.source===OFFICIAL_SOURCE?officialPageData(db,url):null;
 ...
 if(url.pathname==='/'){
  title='AI订阅比价与降价提醒｜ChatGPT、Claude、Gemini · AI订阅价格雷达';
  description='比较 ChatGPT、Claude、Gemini 等 AI 订阅公开报价，查看官方地区价格参考、交付方式与期限，保存降价和到期提醒。按同条件比价，回原渠道核验。';
  ...
  if(directory){
   ...
   if(chosen?.key==='chatgpt-pro-5x'){
    title='ChatGPT Pro 5x 价格比较：期限、地区与充值渠道 · AI订阅价格雷达';
    description='比较 ChatGPT Pro 5x 多家店铺报价，分别核对期限、地区、币种与充值渠道；了解 5x 和 20x 的档位区别及当前开通条件，回原渠道确认实付与续费。';
   }
  }
 }
 ...
 if(official){
  const actualPage=Number(html.match(/data-official-page="(\d+)"/)?.[1]||1);
  const filtered=['region','currency','period','purchase'].some(k=>url.searchParams.get(k)&&url.searchParams.get(k)!=='all');
  canonical=SITE_ORIGIN+(official.product?officialProductHref(official.product.id):OFFICIAL_PATH);
  if(official.product&&actualPage>1&&!filtered)canonical+='&page='+actualPage;
  title=(official.product?official.product.name+' 官方地区价格'+(actualPage>1?' · 第'+actualPage+'页':''):'AI 官方订阅地区价格｜ChatGPT、Claude 等套餐')+' · AI订阅价格雷达';
  description=official.product?'查看 '+official.product.name+' 的 App Store 地区内购参考，核对原币标价、人民币参考额、周期、来源和刷新时间。当前数据来自 CardNav 汇总，以实际结账金额和购买资格为准。':'按套餐查看 ChatGPT Plus、ChatGPT Go、Claude Pro 等 AI 官方订阅的 App Store 地区价格参考，保留原币、来源和时间，并说明周期与汇率资料缺口。';
 }
 ...
 const publisher={'@type':'Organization','@id':SITE_ORIGIN+'/#publisher',name:'AIradar · AI订阅价格雷达',url:SITE_ORIGIN+'/',logo:SITE_ORIGIN+'/assets/brands/airadar.png'};
 const schema=[{'@context':'https://schema.org','@type':'WebSite','@id':SITE_ORIGIN+'/#website',url:SITE_ORIGIN+'/',name:'AI订阅价格雷达',inLanguage:'zh-CN',publisher:{'@id':publisher['@id']}},{'@context':'https://schema.org','@type':url.pathname==='/'||url.pathname===OFFICIAL_PATH||content&&!article?'CollectionPage':'WebPage',url:canonical,name:title,description,inLanguage:'zh-CN',isPartOf:{'@id':SITE_ORIGIN+'/#website'}}];
 schema.push({'@context':'https://schema.org',...publisher});
 if(article){schema.push({'@context':'https://schema.org','@type':'Article',url:canonical,headline:article.title,description:article.description,datePublished:article.published,dateModified:article.updated,inLanguage:'zh-CN',mainEntityOfPage:canonical,author:{'@type':'Organization',name:article.author,url:SITE_ORIGIN+'/'},publisher:{'@id':publisher['@id']}});schema.push({'@context':'https://schema.org','@type':'BreadcrumbList',itemListElement:[{'@type':'ListItem',position:1,name:'首页',item:SITE_ORIGIN+'/'},{'@type':'ListItem',position:2,name:article.section.label,item:SITE_ORIGIN+article.section.path},{'@type':'ListItem',position:3,name:article.title,item:canonical}]});}
 if(official){
  if(official.product){
   const source=official.product.rows.find(r=>r.sourceUrl)?.sourceUrl;
   schema.push({'@context':'https://schema.org','@type':'Dataset',name:official.product.name+' App Store 地区价格参考',url:canonical,description,inLanguage:'zh-CN',creator:{'@type':'Organization',name:'CardNav',url:'https://cardnav.xyz/'},publisher:{'@id':publisher['@id']},isAccessibleForFree:true,measurementTechnique:'整理 CardNav 公开汇总记录，未经本站逐条结账核验',...(source?{isBasedOn:source}:{}),variableMeasured:['国家或地区','原币标价','原币币种','人民币参考额','订阅周期（未提供时标明）','购买渠道','上游刷新时间','汇率日期（未提供时标明）']});
   schema.push({'@context':'https://schema.org','@type':'BreadcrumbList',itemListElement:[{'@type':'ListItem',position:1,name:'首页',item:SITE_ORIGIN+'/'},{'@type':'ListItem',position:2,name:'官方地区价',item:SITE_ORIGIN+OFFICIAL_PATH},{'@type':'ListItem',position:3,name:official.product.name,item:canonical}]});
  }else if(official.products.length)schema.push({'@context':'https://schema.org','@type':'ItemList',itemListElement:official.products.map((p,i)=>({'@type':'ListItem',position:i+1,name:p.name,url:SITE_ORIGIN+officialProductHref(p.id)}))});
 }
 if(product&&!official)schema.push({'@context':'https://schema.org','@type':'BreadcrumbList',itemListElement:[{'@type':'ListItem',position:1,name:'AI订阅价格比较',item:SITE_ORIGIN+'/'},{'@type':'ListItem',position:2,name:publicProductName(product),item:canonical}]});
 ...
 const tags=`<link rel="canonical" href="${esc(canonical)}"><meta name="description" content="${esc(description)}"><meta name="robots" content="index, follow, max-snippet:-1, max-image-preview:large"><meta property="og:title" content="${esc(title)}"><meta property="og:description" content="${esc(description)}"><meta property="og:type" content="${article?'article':'website'}"><meta property="og:url" content="${esc(canonical)}"><meta property="og:locale" content="zh_CN"><meta property="og:image" content="${SITE_ORIGIN}/assets/brands/airadar.png"><meta property="og:image:type" content="image/png"><meta property="og:image:width" content="512"><meta property="og:image:height" content="512"><meta property="og:image:alt" content="AIradar · AI订阅价格雷达标志"><meta name="twitter:card" content="summary"><meta name="twitter:image" content="${SITE_ORIGIN}/assets/brands/airadar.png"><meta name="twitter:image:alt" content="AIradar · AI订阅价格雷达标志"><meta name="twitter:title" content="${esc(title)}"><meta name="twitter:description" content="${esc(description)}">${verification}<script type="application/ld+json">${JSON.stringify(schema).replace(/</g,'\\u003c')}</script>`;
