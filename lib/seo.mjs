...
export function sitemap(db){const urls=[SITE_ORIGIN+'/',SITE_ORIGIN+'/sources',SITE_ORIGIN+'/privacy',...seoProducts(db).map(productUrl)];
...
export function seoRoute(req,res,url,db){
 ...
 if(['/index.html','/product/','/sources/','/privacy/','/alerts/'].includes(url.pathname)&&['GET','HEAD'].includes(req.method)){res.writeHead(301,{Location:(url.pathname==='/index.html'?'/':url.pathname.slice(0,-1))+url.search});res.end();return true;}
...
export function decorateSeo(html,url,db,status=200){
 ...
 let description=({'/':'比较 ChatGPT、Claude、Gemini 等 AI 订阅的公开报价、库存与历史价格，按交易渠道筛选并前往原店核验。本站不售卖账号，不提供交易担保。','/sources':'了解 AI 订阅价格雷达的数据来源、公开采集范围与更新时间，核对原店直采和聚合来源的差异。','/alerts':'查看 AI 订阅公开报价变化提醒，结合历史快照与原店信息核验价格。','/privacy':'了解 AI 订阅价格雷达的访问统计口径、数据保留期限及主动投稿的隐私保护方式。'})[url.pathname]||'AI 订阅公开报价与历史快照参考。';
 if(product){canonical=productUrl(product);const actualPage=Number(html.match(/第 (\d+) \/ \d+ 页/)?.[1]||1);if(actualPage>1)canonical+='&page='+actualPage;title=`${product.name||product.product_id}价格与报价 · ${sourceNames[product.source]||product.source}${actualPage>1?' · 第'+actualPage+'页':''} · AI 订阅价格雷达`;description=`查看${product.name||product.product_id}的${sourceNames[product.source]||product.source}公开报价、库存状态和历史最低价${actualPage>1?'，报价第'+actualPage+'页':''}。报价仅供比较，请到原店核验规格、价格与质保，本站不提供交易担保。`;}
