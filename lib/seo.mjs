...
export function sitemap(db,directory=[]){const urls=[SITE_ORIGIN+'/',SITE_ORIGIN+'/sources',SITE_ORIGIN+'/privacy',SITE_ORIGIN+'/advertise'];
...
export function seoRoute(req,res,url,db,loadDirectory=()=>[]){
 ...
 if(url.pathname.startsWith('/api/')||['/submit','/submit-shop'].includes(url.pathname))res.setHeader('X-Robots-Tag','noindex, nofollow');
...
export function decorateSeo(html,url,db,status=200){
 const privatePage=['/submit','/submit-shop'].includes(url.pathname)||status>=400;
 ...
 if(url.pathname==='/'){
  ...
  description='按品牌浏览 AI 订阅、API 中转及邮箱接码的产品，直接查看多家店铺的公开报价、商品规格与交易平台，支持价格排序。价格以原店结算页面为准。';
