...
const CATALOG_REQUESTS=[
 '建站系统名称及版本；如果不清楚，可提供系统官网或介绍页。',
 '无需登录的公开商品列表链接，以及 1–2 个目前在售的商品详情页链接。',
 '如有公开只读商品接口，请提供文档、目录路径或脱敏返回样例；没有接口也可直接说明，由我们评估适配。',
];
const REQUESTS={
 not_found:CATALOG_REQUESTS,
 unsupported_platform:CATALOG_REQUESTS,
 invalid_catalog:[...CATALOG_REQUESTS,'如有分页或多规格商品，请附公开的分页参数、价格/库存字段及规格说明。'],
 ...
 redirect_disallowed:['店铺最终使用的 HTTPS 主页及公开商品列表链接，不附带登录信息或访问令牌。','如近期更换域名或商品目录路径，请简要说明变更。'],
...
export function preflightFailureNotice(application,preflight){
 ...
 const checklist=requests
  ? '\n\n请按以下清单回复，已有的资料先提供即可：\n'+requests.map((item,i)=>`${i+1}. ${item}`).join('\n')+'\n\n收到后由站方核对读取方式和商品信息，再安排适配或重测；暂不承诺接入时间或收录结果。'
...
export function queuePreflightResultMail(db,application,preflight,{now=new Date()}={}){
 ...
 const eventKey=ready?application.id+':result:'+preflight.id
  :`${application.id}:auto-preflight:v1:${application.version}:${notice.code}:${recipientKey}`;
 const owned=!db.isTransaction;
 if(owned)db.exec('BEGIN IMMEDIATE');
 try{
  // Retire old automatic outcomes even when this outcome was already mailed before.
  // Explicit operator replies and application receipts are never changed here.
  db.prepare(`UPDATE merchant_mail_outbox SET status='superseded' WHERE application_id=? AND event_key<>?
    AND status IN ('queued','retry','failed') AND (event_key LIKE ? OR (public_reply='' AND (event_key LIKE ? OR event_key LIKE ?)))`)
   .run(application.id,eventKey,application.id+':auto-preflight:%',application.id+':result:%',application.id+':testing:%');
  const queued=queueMerchantMail(db,{id:application.id,email:application.email,stage:notice.stage,publicReply:notice.publicReply,eventKey,now});
