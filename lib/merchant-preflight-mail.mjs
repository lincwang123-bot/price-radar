...
export function queuePreflightResultMail(db,application,preflight,{now=new Date()}={}){
 ...
 const eventKey=`${application.id}:auto-preflight:v1:${application.version}:${notice.code}:${recipientKey}`;
 const owned=!db.isTransaction;
 if(owned)db.exec('BEGIN IMMEDIATE');
 try{
  // The shared outbox policy owns supersession and the lifetime two-mail cap.
