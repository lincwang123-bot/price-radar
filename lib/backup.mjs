...
export function backupStatus(directory) {
  try {
    const saved=JSON.parse(readFileSync(path.join(directory, "status.json"), "utf8"));
    const ageHours=(Date.now()-Date.parse(saved.createdAt))/3600000;
    const stale=!Number.isFinite(ageHours)||ageHours>36||ageHours<0;
    if(typeof saved.file!=='string'||!NAME.test(saved.file)||path.basename(saved.file)!==saved.file) throw new Error('备份文件名不安全');
    const file=path.join(directory,saved.file), info=lstatSync(file);
    if(!info.isFile()||!Number.isSafeInteger(saved.bytes)||saved.bytes<=0||info.size!==saved.bytes) throw new Error('备份文件大小不匹配');
    let integrity='size-only';
    if(saved.sha256!==undefined) {
      if(!/^[a-f0-9]{64}$/.test(saved.sha256)||createHash('sha256').update(readFileSync(file)).digest('hex')!==saved.sha256) throw new Error('备份文件摘要不匹配');
      integrity='sha256';
    }
    return {...saved,offsite:false,integrity,ok:saved.ok===true&&!stale,stale,ageHours:Number.isFinite(ageHours)?Math.round(ageHours):null};
  } catch { return { ok: false, integrity:'unverified', message: "备份缺失或文件校验失败", offsite: false }; }
}
...
export async function backupSubmissions(dbPath, directory, { keep = 14, now = new Date(),kind="submissions" } = {}) {
  ...
  try {
    ...
    const status = { ok: true, createdAt: now.toISOString(), file: name, bytes: statSync(file).size, sha256:createHash('sha256').update(readFileSync(file)).digest('hex'), ...verified, offsite: false, keep };
