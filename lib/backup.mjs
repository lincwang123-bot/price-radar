...
const NAME = /^submissions-\d{8}T\d{9}Z-[a-f0-9]+\.sqlite$/;
const TABLES = ["feedback_submissions", "cooperation_submissions", "submission_actions"];
function counts(db) {
  return Object.fromEntries(TABLES.filter(t => db.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name=?").get(t))
...
export function verifyBackup(file) {
  ...
  try {
    ...
    for (const t of TABLES.slice(0, 2)) if (!db.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name=?").get(t)) throw new Error("备份缺少投稿表");
    return { quickCheck: "ok", counts: counts(db) };
...
export async function backupSubmissions(dbPath, directory, { keep = 14, now = new Date() } = {}) {
  ...
  const name = `submissions-${stamp}-${randomBytes(6).toString("hex")}.sqlite`;
  ...
  try {
    ...
    const old = readdirSync(directory).filter(n => NAME.test(n)).sort().reverse().slice(keep);
