...
export function mailSummary(mail={items:[]}) {
  ...
  if(row?.status==='superseded'&&row.errorCode?.startsWith('mail_policy_'))return ({mail_policy_limit:'未发送：已使用自动通知名额',mail_policy_stage:'已取消：不再发送此阶段通知',mail_policy_replaced:'已取消：由当前结果通知替代'}[row.errorCode]||'已取消：两封自动通知规则');
