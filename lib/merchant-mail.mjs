...
import {initMerchantReplySchema,latestMerchantReply,listMerchantReplies} from './merchant-replies.mjs';
...
export function initMerchantMailSchema(db) {
  ...
  if (!db.prepare('PRAGMA table_info(merchant_mail_outbox)').all().some(row=>row.name==='reply_round_id')) db.exec('ALTER TABLE merchant_mail_outbox ADD COLUMN reply_round_id INTEGER NOT NULL DEFAULT 0');
  initMerchantReplySchema(db);
  db.exec('CREATE INDEX IF NOT EXISTS merchant_mail_application ON merchant_mail_outbox(application_id,id)');
}
export function queueMerchantMail(db,{id,email,stage,eventKey,publicReply='',replyRoundId,now=new Date()}) {
  ...
  const currentRound=merchant?latestMerchantReply(db,id)?.id||0:0;
  const round=['approved','need_info'].includes(stage)?replyRoundId??currentRound:0;
  if(!Number.isSafeInteger(round)||round<0||(round!==0&&round!==currentRound)){
    if(owned)db.exec('COMMIT');return false;
  }
  const inserted=db.prepare('INSERT OR IGNORE INTO merchant_mail_outbox(event_key,application_id,recipient,stage,created_at,next_attempt_at,public_reply,reply_round_id) VALUES(?,?,?,?,?,?,?,?)').run(eventKey,id,email,stage,stamp,stamp,publicReply,round).changes;
  // One initial receipt/outcome, then one outcome per verified, deduplicated reply round.
...
export function merchantMailStatus(db,id) {
  ...
  return {replies:listMerchantReplies(db,id),items:db.prepare(`SELECT id,stage,status,attempts,created_at AS createdAt,accepted_at AS acceptedAt,error_code AS errorCode,public_reply AS publicReply,reply_round_id AS replyRoundId FROM merchant_mail_outbox WHERE application_id IN (${ids.map(()=>'?').join(',')}) ORDER BY id DESC LIMIT 10`).all(...ids)};
}
