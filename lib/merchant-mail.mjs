...
export function queueMerchantMail(db,{id,email,stage,eventKey,publicReply='',now=new Date()}) {
  ...
  if (inserted && !['received','submission_received','reply'].includes(stage)) db.prepare("UPDATE merchant_mail_outbox SET status='superseded' WHERE application_id=? AND event_key<>? AND stage NOT IN ('received','submission_received','reply') AND public_reply='' AND (?<>'published' OR stage<>'approved') AND status IN ('queued','retry','failed')").run(id,eventKey,stage);
  return !!inserted;
}
...
export function merchantMailMessage(row,from,{replyEnabled=false}={}) {
  ...
  return {from:{name:'AirRadar 通知',address:from},to:row.recipient,
