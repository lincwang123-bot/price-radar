...
export function queueMerchantMail(db,{id,email,stage,eventKey,publicReply='',now=new Date()}) {
  ...
  // Receipts and explicit replies are independent events, never swallowed by a rapid state change.
  // Detailed automatic preflight copy is not an explicit reply: retire it with newer outcomes too.
  // Publishing must not erase an approval notification. Pause/rejection may supersede obsolete unsent states.
  if (inserted && !['received','submission_received','reply'].includes(stage)) db.prepare("UPDATE merchant_mail_outbox SET status='superseded' WHERE application_id=? AND event_key<>? AND stage NOT IN ('received','submission_received','reply') AND (public_reply='' OR event_key LIKE ?) AND (?<>'published' OR stage<>'approved') AND status IN ('queued','retry','failed')").run(id,eventKey,id+':auto-preflight:%',stage);
  return !!inserted;
}
export function merchantMailStatus(db,id) {
  return {items:db.prepare('SELECT id,stage,status,attempts,created_at AS createdAt,accepted_at AS acceptedAt,error_code AS errorCode,public_reply AS publicReply FROM merchant_mail_outbox WHERE application_id=? ORDER BY id DESC LIMIT 10').all(id)};
}
