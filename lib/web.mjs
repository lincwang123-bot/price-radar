...
function layout(title, body, active = "") {
  ...
  const categoryScript = active === "home" ? `<script>
    (() => {
      ...
      const groups = Array.from(document.querySelectorAll("#comparison [data-family]"));
...
function sourceProducts(list) {
  ...
  const desktopRows = groups.map((group) => {
    ...
    return `<tr class="table-group-row tone-${group.tone}"><th colspan="4" scope="rowgroup"><span class="group-mark" aria-hidden="true"></span><span class="group-title">${esc(group.label)}</span><span class="group-count">${group.items.length} 个产品</span></th></tr>${rows}`;
  }).join("");
  const mobileGroups = groups.map((group, index) => {
    ...
    return `<details class="catalog-group tone-${group.tone}"${index === 0 ? " open" : ""}><summary class="group-summary"><span class="group-summary-copy"><span class="group-mark" aria-hidden="true"></span><strong>${esc(group.label)}</strong><span class="group-count">${group.items.length} 个产品</span></span><span class="group-chevron" aria-hidden="true">${icon("chevron")}</span></summary><div class="catalog-group-body">${cards}</div></details>`;
  }).join("");
  return `<div class="table-scroll desktop"><table class="catalog-table"><thead><tr><th>产品</th><th class="num">最低价</th><th class="num">有货 / 报价</th><th aria-label="查看详情"></th></tr></thead><tbody>${desktopRows}</tbody></table></div><div class="mobile"><div class="catalog-groups">${mobileGroups}</div></div>`;
}

function sourceSection(list) {
  const label = list ? (SOURCE_LABEL[list.source] ?? list.source) : "数据源";
  const count = list?.products.length ?? 0;
  return `<section class="surface"><div class="head"><div style="display:flex;align-items:center;gap:9px;min-width:0">${sourceBadge(list?.source ?? "neutral")}<div><h2>${esc(label)}</h2><p class="note">${count} 个产品 · 当前快照</p></div></div>${updatedMeta(list)}</div>${sourceProducts(list)}</section>`;
}
