...
<head>
  ...
  <style>${WEB_CSS}${MERCHANT_BADGE_CSS}${MERCHANT_X_CSS}</style><style>${MARKET_CSS}</style><style>${DIRECTORY_CSS}</style><style>${HOME_PRICE_CSS}</style>${analyticsScript}
</head>
<body>
  <header class="app-header"><div class="header-inner"><div class="header-main">
    ...
    <nav class="app-nav" aria-label="主导航">${nav("/", "home", "总览")}${nav("/following", "following", "我的关注")}${nav("/weekly", "weekly", "行情周报")}${nav("/alerts", "alerts", "价格提醒")}${nav("/submit", "submit", "反馈与合作")}<a class="nav-link" data-reader-nav href="/login">登录 / 注册</a></nav>
