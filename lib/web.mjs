import {normalizeMerchantSubmission,resolveMerchantShopUrl} from './merchant-shop-url.mjs';
import { brandMark as platformMark, brandAssetRoute } from "./brand-icons.mjs";
...
import { startMerchantWorkflow } from './merchant-workflow.mjs';
...
import { buildProductDirectory, directoryQuotes, DIRECTORY_DELIVERIES } from "./product-directory.mjs";
import { DIRECTORY_CSS } from "./directory-styles.mjs";
import {contentForPath,contentPage} from './guides.mjs';
import {GUIDE_CSS} from './guide-styles.mjs';
import {officialPriceContent,OFFICIAL_CSS} from './official-price-ui.mjs';
import {createRetention} from './retention.mjs';
import {retentionAssets,retentionDialogs,followingContent,followButton,productReference,weeklyContent,unsubscribeContent} from './retention-ui.mjs';
import {accountAssets,accountContent,loginContent} from './account-ui.mjs';
import {safeAccountReturn} from './account-auth.mjs';
import {profileContact} from './account-profile.mjs';
import {directoryQuoteIdentity} from './product-directory.mjs';
import { offerDelivery } from './offer-spec.mjs';
// 零依赖 Node http + 服务端渲染，公开页面只写入独立的投稿数据库。
// 保持既有路由，并提供 /submit 与 /api/submissions。

import http from "node:http";
import {readFileSync} from 'node:fs';
import {createMerchantX,buildShopCatalog} from './merchant-x.mjs';
import {searchShops,normalizeShopQuery} from './shop-search.mjs';
import {shopSearchForm,shopSearchContent} from './shop-search-ui.mjs';
import {MERCHANT_X_CSS,merchantXBadge,merchantStoreLinks,shopContent,claimContent} from './merchant-x-ui.mjs';
const merchantXScript=readFileSync(new URL('../assets/merchant-x.js',import.meta.url),'utf8');
import {outboundHref,handleOutbound} from "./outbound.mjs";
import {applicationContent,claimLink,sponsoredContent} from "./commerce-ui.mjs";
import {SPONSOR_CSS,sponsorAdvertiseContent,sponsorBlock,sponsorInvite} from "./sponsor-ui.mjs";
import {sponsorEntries,sponsorItems,migrateSponsorScopes} from "./sponsor-service.mjs";
const sponsorScript=readFileSync(new URL("../assets/sponsors.js",import.meta.url),"utf8");
const sponsorArt=new Map(['amber','blue','violet','green'].map(theme=>['/assets/sponsor-'+theme+'.webp',readFileSync(new URL('../assets/sponsor-'+theme+'.webp',import.meta.url))]));
...
const FAMILIES = [
  { family: "ChatGPT Go", platform: "ChatGPT", direct: "chatgpt-go", official: "chatgpt-go", ldxp: null },
  { family: "ChatGPT Plus（试用/成品）", platform: "ChatGPT", direct: "chatgpt-plus", official: null, ldxp: null },
  { family: "ChatGPT Plus（正价代充）", platform: "ChatGPT", direct: "chatgpt-plus-recharge", official: "chatgpt-plus", ldxp: "chatgpt-plus-代充" },
  { family: "ChatGPT Pro 5x", platform: "ChatGPT", direct: "chatgpt-pro-5x", official: "chatgpt-pro-5x", ldxp: null },
  { family: "ChatGPT Pro 20x", platform: "ChatGPT", direct: "chatgpt-pro-20x", official: "chatgpt-pro-20x", ldxp: null },
  { family: "ChatGPT Team/Business", platform: "ChatGPT", direct: "chatgpt-team-business", official: null, ldxp: null },
  { family: "Claude Pro", platform: "Claude", direct: "claude-pro-month", official: "claude-pro", ldxp: "claude-pro-代充" },
  { family: "Claude Max 5x", platform: "Claude", direct: "claude-max-5x", official: "claude-max-5x", ldxp: null },
  { family: "Claude Max 20x", platform: "Claude", direct: "claude-max-20x", official: "claude-max-20x", ldxp: null },
  { family: "Gemini Plus", platform: "Gemini", direct: "gemini-pro-recharge", official: "gemini-ai-plus", ldxp: null },
  { family: "Gemini Ultra", platform: "Gemini", direct: "gemini-ultra", official: "gemini-ai-ultra", ldxp: null },
  { family: "Grok Super", platform: "Grok", direct: "super-grok", official: "grok-supergrok", ldxp: null },
  { family: "Grok Super Heavy", platform: "Grok", direct: "super-grok-heavy", official: "grok-supergrok-heavy", ldxp: null },
  { family: "X Premium", platform: "X", direct: "x-twitter-premium", official: "x-premium", ldxp: null },
];
...
function latestProducts(db, source, channel = "all", framework = "all") {
  ...
  ).all(source, latest.snapshot_id).filter(p=>!retiredCatalogItem(p));
  if (SHOP_SOURCES.has(source)) products=products.flatMap(p=>{
    ...
    return [{...projected,visible_offer_count:offers.length}];
  });
...
function metricLine(offer) {
  ...
  const channelText=`<span class="dim">${esc(offer.source==="cardnav-official"?"官方渠道":channel.label)}</span><span class="quote-time" title="${esc(time.absolute||'')}">${esc(time.relative)}${time.staleLabel?' · '+esc(time.staleLabel):''}</span>`;
...
function icon(name, cls = "") {
  ...
  if (name === "warning") return svg('<path d="m12 3 10 18H2L12 3Z"/><path d="M12 9v4m0 4h.01"/>');
...
function layout(title, body, active = "", options = {}) {
  ...
  const categoryLink = ({key,label,platform})=>{
    ...
    return '<a href="/?'+esc(params.toString())+'" class="product-filter'+(selected?' active':'')+'" data-family-filter="'+key+'" aria-current="'+(selected?'page':'false')+'"><span class="product-filter-mark mark-'+key+'" aria-hidden="true">'+platformMark(platform||key)+'</span><span>'+label+'</span></a>';
  };
  ...
  const analyticsScript = options.privateAccount ? '' : cloudflareWebAnalytics();
  ...
  const friendLinks = options.showFriendLinks ? '<nav class="footer-actions" aria-label="友情链接"><span>友情链接：</span><a href="https://www.ztsite.com/" target="_blank" rel="noopener noreferrer">ZT折腾簿</a></nav>' : "";
...
<head>
  ...
  ${headExtra}${retentionAssets}${accountAssets}
  <title>${esc(title)} · AI 订阅价格雷达</title>
  <style>${WEB_CSS}${MERCHANT_BADGE_CSS}${MERCHANT_X_CSS}${SPONSOR_CSS}</style><style>${MARKET_CSS}</style><style>${DIRECTORY_CSS}</style><style>${HOME_PRICE_CSS}</style><style>${GUIDE_CSS}</style>${active==='official'?'<style>'+OFFICIAL_CSS+'</style>':''}${analyticsScript}
</head>
<body>
  <header class="app-header"><div class="header-inner"><div class="header-main">
    ...
    <nav class="app-nav" aria-label="主导航">${nav("/", "home", "总览")}${nav("/official-prices", "official", "官方地区价")}${nav("/following", "following", "我的关注")}${nav("/weekly", "weekly", "行情周报")}${nav("/guides", "guides", "订阅指南")}${nav("/alerts", "alerts", "价格提醒")}${nav("/submit", "submit", "反馈与合作")}<a class="nav-link" data-reader-nav href="/login">登录 / 注册</a></nav>
  ...
  <footer class="site-footer" id="site-disclaimer"><div class="footer-inner"><div><p class="footer-disclaimer">本站仅提供公开数据的汇总展示；价格、库存、服务内容及交易结果以原站实际页面为准，本站不对第三方信息的准确性、完整性或由此产生的交易结果承担责任。品牌标识仅用于产品识别，不代表官方合作。</p><nav class="footer-actions" aria-label="数据反馈与合作"><a href="/help">帮助中心</a><a href="/guides">订阅指南</a><a href="/submit-shop">提交店铺</a><a href="/claim-shop?new=1">店主认领 X</a><a href="/submit?type=feedback">数据反馈</a><a href="/submit?type=cooperation">供需提交</a></nav>${friendLinks}</div><div class="footer-contact" aria-label="联系站长"><span class="footer-contact-label">联系站长</span><span class="footer-contact-links"><a class="footer-contact-link" href="https://x.com/superwang" target="_blank" rel="noopener noreferrer" aria-label="在 X 联系站长 @superwang"><b>X</b>@superwang</a><a class="footer-contact-link" href="https://t.me/lincwang" target="_blank" rel="noopener noreferrer" aria-label="在 Telegram 联系站长 @lincwang"><b>TG</b>@lincwang</a></span></div></div></footer>${storeRiskModal}${categoryScript}${storeRiskScript}
${retentionDialogs()}</body>
</html>`;
}
...
function overviewContent(db, direct, cardnav, ldxp) {
  const entries = OVERVIEW_FAMILIES.map((summary) => {
    ...
    const linked = directOffer ?? official ?? supplier;
    const updatedList = directOffer ? direct : (official ? cardnav : (supplier ? ldxp : null));
    const sourceCount = [official, directOffer, supplier].filter(Boolean).length;
    return {
      ...summary,
...
function familyContent(db, direct, cardnav, ldxp) {
  ...
  const desktop = groups.map((group) => {
    const rows = group.items.map((item) => `<tr data-family="${group.tone}">
      ...
      <td class="num">${priceStack(item.official)}</td><td class="num">${priceStack(item.direct)}</td><td class="num">${priceStack(item.supplier)}</td>
    </tr>`).join("");
  ...
  const mobile = `<div class="family-groups">${groups.map((group, index) => {
    const cards = group.items.map((item) => `<article class="family-card"><div class="family-head"><strong>${esc(item.family.family)}</strong><span>${esc(item.family.platform)}</span></div>${mobilePriceLine("官方区最低", item.official)}${mobilePriceLine("原店直采最低", item.direct)}${mobilePriceLine("LDXP 货源最低", item.supplier)}</article>`).join("");
...
function sourceProductGroups(list, products) {
  ...
  const order = ["ChatGPT", "Claude", "Gemini", "Grok", "X", "Cursor", "Perplexity", "Notion AI", "Manus", "Microsoft", "其他", "邮箱"];
...
function alertList(rows) {
  rows=rows.filter(row=>!retiredCatalogItem(row));
...
function marketCatalog(entries,title,{business=false}={}) {
  ...
  const groups=new Map();for(const e of entries){const platform=e.list.source==="ldxp-goods"?"关键词产品":e.product.platform||"其他";if(!groups.has(platform))groups.set(platform,[]);groups.get(platform).push(e);}
  ...
  const body=[...groups].map(([platform,items])=>{const tone=platform==="关键词产品"?"other":platformTone(platform);
...
const PURPOSES=[['all','全部用途'],['recharge','订阅代充'],['account','成品账号'],['shared','共享 / 合租'],['code','兑换码 / 卡密'],['other','其他 / 待确认']];
...
function purposeOf(product){const text=[product.name,product.product_type,product.selected_offer?.title].join(' ');if(/共享|合租|拼车/.test(text))return 'shared';if(/代充|直充|直冲/.test(text))return 'recharge';if(/成品|账号/.test(text))return 'account';if(/卡密|兑换|CDK/i.test(text))return 'code';return 'other';}
function directoryHref(state, changes={}) {
  const params=new URLSearchParams();
  for(const [key,value] of Object.entries({...state,...changes})) if(['family','product','sort','channel','spec','currency','page','delivery'].includes(key)&&value!=null&&value!=='')params.set(key,String(value));
...
function directoryDeliveryLinks(product, family, quotes) {
  const render = group => {
    const result=directoryQuotes(product,{delivery:group.kind});
    const price=['recharge','account','shared','seat'].includes(group.kind)?directoryMinimumHtml(result):'<span class="directory-delivery-count">'+group.count+' 条报价 · 查看</span>';
    const slot=['recharge','account'].includes(group.kind)?' data-delivery-slot="'+group.kind+'"':'';
    return '<a class="directory-delivery-card" data-delivery-link="'+group.kind+'"'+slot+' href="'+directoryHref({family,product:product.key,delivery:group.kind})+'"><span class="directory-delivery-label">'+esc(group.label)+'</span>'+price+'<span class="directory-delivery-cta">查看店铺 ›</span></a>';
  };
  const primary=[['recharge','代充'],['account','成品号']].map(([kind,label])=>{
    const group=quotes.deliveries.find(group=>group.kind===kind);
    return group?render(group):'<div class="directory-delivery-card directory-delivery-placeholder" data-delivery-slot="'+kind+'"><span class="directory-delivery-label">'+label+'</span><span class="directory-minimum-empty">暂无报价</span></div>';
  });
  const other=quotes.deliveries.filter(group=>!['recharge','account'].includes(group.kind));
  return '<div class="directory-delivery-links">'+primary.join('')+(other.length?'<details class="directory-delivery-more"><summary>其他交付 · '+other.reduce((n,g)=>n+g.count,0)+' 条</summary><div>'+other.map(render).join('')+'</div></details>':'')+'</div>';
}

function directoryQuoteRow(entry, merchantBadges = [], xProfiles = []) {
  ...
  const fullSpec=offer.comparison_known===false?offer.comparison_label||'规格待确认':offer.comparison_label||product.comparison_label||publicSpec(product.spec)||'规格待确认';
  const deliveryWords=['代充','成品账号','成品号','共享','共享或合租','席位','卡密','卡密（用途待确认）','交付未注明','交付待确认'];
  const spec=entry.showDelivery&&!reference?fullSpec.split(' · ').filter(part=>!deliveryWords.includes(part)).join(' · ')||'规格待确认':fullSpec;
  const label=reference?'官方参考':channel?.label||'未确认渠道';
  ...
  return '<article class="directory-quote" data-directory-quote data-price="'+esc(offer.price)+'" data-currency="'+esc(offer.currency||product.currency)+'"><div class="directory-quote-main"><h2>'+esc(offer.store_name|| (reference?label:'店铺信息待确认'))+(!reference?merchantBadgeForOffer({...offer,source:list.source},merchantBadges)+merchantXBadge({...offer,source:list.source},xProfiles):'')+'</h2>'+(!reference?merchantStoreLinks({...offer,source:list.source}):'')+'<p class="directory-quote-spec">'+(entry.showDelivery&&!reference?'<strong class="quote-delivery">'+esc(entry.delivery.label+(entry.delivery.conflict?'（描述有冲突）':''))+'</strong> · ':'')+esc(spec)+'</p>'+(offer.title&&offer.title!==spec?'<p class="directory-quote-title">'+esc(offer.title)+'</p>':'')+'<p class="directory-quote-meta">'+esc(label)+' · '+esc(reference?'参考报价':listingState(offer,list.stale))+' · 报价时间 '+esc(fmtTime(quoteTimeInfo(offer).absolute))+'</p></div><div class="directory-quote-buy"><span class="money">'+fmtPrice(offer.price,offer.currency||product.currency)+'</span>'+(href?'<a class="offer-shop-button" data-store-risk data-outbound-target="'+target+'" href="'+esc(href)+'" target="_blank" rel="noopener noreferrer">'+(reference?'查看原站':'前往店铺')+' '+icon('external')+'</a>':'<span class="note">'+(reference?'仅作价格参考':'暂无店铺链接')+'</span>')+'<a class="directory-history" href="'+history+'">价格记录</a></div></article>';
}
function directoryQuotePage(category,selected,url,{db,analytics,req,res,merchantBadges=[],xProfiles=[]}={}) {
  const initial=directoryQuotes(selected,{channel:normalizeChannel(url.searchParams.get('channel')),currency:url.searchParams.get('currency')||''});
  const requestedDelivery=url.searchParams.get('delivery');
  const specDelivery=initial.entries.find(entry=>!entry.reference&&entry.offer.comparison_key===url.searchParams.get('spec'))?.delivery.kind;
  const activeDelivery=selected.deliveryEnabled ? (DIRECTORY_DELIVERIES.some(group=>group.kind===requestedDelivery)?requestedDelivery:specDelivery||initial.deliveries[0]?.kind||'unknown') : '';
  const state={family:category.key,product:selected.key,delivery:activeDelivery,sort:['desc','price_desc'].includes(url.searchParams.get('sort'))?'price_desc':['asc','price','price_asc'].includes(url.searchParams.get('sort'))?'price_asc':'comprehensive',channel:normalizeChannel(url.searchParams.get('channel')),spec:url.searchParams.get('spec')||'',currency:url.searchParams.get('currency')||''};
  const result=directoryQuotes(selected,{...state,merchantBadges,xProfiles}),shops=result.entries.filter(entry=>!entry.reference),references=result.entries.filter(entry=>entry.reference);
  ...
  const deliveryControls=selected.deliveryEnabled&&initial.deliveries.length?'<nav class="spec-selector delivery-selector" aria-label="交付方式">'+initial.deliveries.map(group=>'<a class="spec-choice" data-delivery-choice="'+group.kind+'" aria-current="'+(state.delivery===group.kind)+'" href="'+directoryHref(state,{delivery:group.kind,spec:'',page:''})+'">'+esc(group.label)+' · '+group.count+'</a>').join('')+'</nav>':'';
  const sortLabel=state.sort==='comprehensive'?'综合排序':state.sort==='price_desc'?'价格从高到低':'价格从低到高';
  const sortControls='<div class="quote-sort"><nav class="quote-sort-options" aria-label="报价排序">'+[['comprehensive','综合排序'],['price_asc','价格从低到高'],['price_desc','价格从高到低']].map(([value,label])=>'<a class="quote-sort-choice" data-sort-choice="'+value+'" aria-current="'+(state.sort===value)+'" href="'+directoryHref(state,{sort:value,page:''})+'">'+esc(label)+'</a>').join('')+'</nav><p class="quote-sort-note">'+(state.sort==='comprehensive'?'优先 X 已关联，其次为店铺已收录；同类按价格从低到高。':'按当前筛选条件，'+sortLabel+'。')+'</p></div>';
  const controls=deliveryControls+sortControls+'<div class="quote-controls"><div class="spec-selector" aria-label="交易平台">'+CHANNELS.map(channel=>choice('channel',channel.id,channel.label)).join('')+'</div>'+((result.specs.length>1||result.currencies.length>1||state.spec||state.currency)?'<details class="detail-filters"'+(state.spec||state.currency?' open':'')+'><summary>规格与币种筛选</summary><div class="spec-selector" aria-label="规格筛选">'+choice('spec','','全部规格')+result.specs.map(spec=>choice('spec',spec.key,spec.label+' · '+spec.count)).join('')+'</div><div class="spec-selector" aria-label="币种筛选">'+choice('currency','','全部币种')+result.currencies.map(currency=>choice('currency',currency,currency)).join('')+'</div></details>':'')+'</div>';
  let lastCurrency;
  const rows=shown.map(entry=>{const currency=entry.offer.currency||entry.product.currency;const heading=currency!==lastCurrency?'<h2 class="quote-currency">'+esc(currency)+' 报价</h2>':'';lastCurrency=currency;return heading+directoryQuoteRow(entry,merchantBadges,xProfiles);}).join('');
  ...
  const entries=sponsorEntries([{...category,products:[selected]}]);
  const sponsored=pageSponsors(entries,{db,analytics,req,res},'sponsored_product',selected.key);
  ...
  return '<a class="breadcrumb" href="/?family='+encodeURIComponent(category.key)+'">'+icon('back')+'返回 '+esc(category.label)+'</a><div class="directory-intro"><h1>'+platformMark(category.key)+esc(selected.name)+'</h1><p>'+directoryCount(result,references.length)+(shops.length?' · '+sortLabel:'')+'</p><p>不同周期、交付方式的总价不可直接比较；价格与库存以店铺结算页为准。</p>'+(result.currencies.length>1?'<p>不同币种分别排序，不做汇率换算。</p>':'')+'</div>'+controls+(!shops.length&&references.length?'':'<div class="directory-quotes">'+(rows||'<p class="directory-empty">暂无可用报价，请调整筛选或稍后再来。</p>')+'</div><p class="note">第 '+page+' / '+pageCount+' 页 · 每页 20 条店铺报价</p>'+pagination)+sponsored+(references.length?'<section class="directory-references"><h2>参考报价</h2><p class="note">以下为官方订阅参考，不计入店铺报价。</p>'+references.map(entry=>directoryQuoteRow(entry)).join('')+'</section>':'')+unavailable;
}
function directoryRiskNotice() {
  return '<aside class="directory-risk-note" role="note" aria-label="交易风险提醒">'+icon('warning')+'<p><strong>交易风险提醒</strong>本站仅提供信息导航，不参与交易或售后；收录不代表交易安全保证。下单前核实商品规格与售后条款，警惕低价诱导和私下转账。</p></aside>';
}
function loadDirectory(db) {
  const lists=["direct-shops","ldxp-goods","cardnav-official"].map(source=>{
  ...
  return buildProductDirectory(lists);
}
function pageSponsors(entries,context,placement,pageKey){
  let items=[];try{items=sponsorItems(entries,context.analytics,{placement,pageKey,db:context.db});}catch{return '';}
  if(context.req?.method==='GET'&&items.length)context.res?.once('finish',()=>{if(context.res.statusCode===200)for(const item of items)try{context.analytics.outbound.recordImpression(context.req,item.campaign);}catch{}});
  return items.length?sponsorBlock(items,{placement}):sponsorInvite(placement.replace('sponsored_',''));
}
function homePage(db, url = new URL("http://localhost"),context={}) {
  context={...context,db};
  const filters=filtersOf(url);
  const directory=loadDirectory(db);
  const shopQuery=normalizeShopQuery(url.searchParams.get('shop_q'));
  if(shopQuery){
    const result=searchShops(buildShopCatalog(directory).values(),shopQuery,{page:url.searchParams.get('page')});
    return layout('搜索店铺','<section class="directory">'+shopSearchForm(result.q)+shopSearchContent(result,context)+'</section>','home',{filters:{family:'all'}});
  }
  ...
  const categoryRow=item=>'<section class="directory-category" data-category="'+esc(item.key)+'"><h2 class="directory-category-title">'+platformMark(item.key)+esc(item.label)+'</h2><div class="directory-list">'+item.products.map(product=>{const quotes=directoryQuotes(product),references=quotes.entries.filter(entry=>entry.reference).length;return '<article class="directory-row" data-directory-product="'+esc(product.key)+'"><div class="directory-product-summary"><h3>'+platformMark(item.key)+esc(product.name)+'</h3><p class="directory-count">'+directoryCount(quotes,references,{compact:true})+'</p></div><div class="directory-row-action">'+(product.deliveryEnabled&&quotes.deliveries.length?directoryDeliveryLinks(product,item.key,quotes):directoryMinimumHtml(quotes)+'<a class="directory-action" href="'+directoryHref({family:item.key,product:product.key})+'">'+(quotes.total===references&&references?'查看参考':quotes.total?'查看店铺':'查看记录')+' '+icon('chevron')+'</a>')+'</div></article>';}).join('')+(item.products.length?'':'<p class="directory-empty">暂无可用报价，请稍后再来。</p>')+'</div></section>';
  let title='AI 订阅报价',content;
  if(!category){
    content='<div class="directory-intro"><h1>AI 订阅报价</h1><p>找到产品，一次查看各家店铺的价格与规格。</p></div>'+directoryRiskNotice()+directory.filter(item=>item.primary).map(categoryRow).join('')+'<details class="directory-more"><summary>更多分类</summary>'+directory.filter(item=>!item.primary).map(categoryRow).join('')+'</details>';
  }else if(selected){
    title=selected.name+' · 店铺报价';
    content=directoryQuotePage(category,selected,url,context);
    if(context.retention){
      content=content.replace('</h1>','</h1>'+followButton(selected.key,url.searchParams.get('spec')||''));
      content+=productReference(context.retention,selected.key,{comparisonKey:url.searchParams.get('spec')||''});
    }
  }else{
    title=category.label+' · 产品';
    content='<a class="breadcrumb" href="/">'+icon('back')+'返回全部产品</a><div class="directory-intro"><h1>'+platformMark(category.key)+esc(category.label)+'</h1><p>点击产品，查看各家店铺报价。</p></div>'+directoryRiskNotice()+categoryRow(category);
  }
  if(!selected)content+=pageSponsors(sponsorEntries(category?[category]:directory),context,category?'sponsored_category':'sponsored_home',category?.key||'home');
  ...
  return layout(title,'<section class="directory"'+identity+'>'+shopSearchForm()+content+'</section>','home',{filters:{family:filters.family},showFriendLinks:!category});
}
...
function offerContent(offers, { source, id, page, stale=false, merchantBadges=[], xProfiles=[] }) {
  const state=o=>source==='cardnav-official'?'官方参考':listingState(o,stale);
  ...
  const desktopRows = shown.map((offer, index) => {
    ...
    return `<tr><td class="num offer-rank"><span class="offer-rank-index">#${rank}</span></td><td class="num"><span class="money">${fmtPrice(offer.price, offer.currency)}</span></td><td>${statusMark(state(offer))}</td><td><div class="offer-title"><strong>${esc(offer.store_name || "店铺信息待确认")}${merchantBadgeForOffer({...offer,source},merchantBadges)}${merchantXBadge({...offer,source},xProfiles)}</strong><span>${esc(offer.title ?? "")}</span>${metricLine(offer)}${merchantStoreLinks({...offer,source})||claimLink(offer)}</div></td><td class="dim">${esc(quoteTimeInfo(offer).absolute ? fmtTime(quoteTimeInfo(offer).absolute) : "—")}</td><td class="num offer-action">${link}</td></tr>`;
  }).join("");
  const mobileCards = shown.map((offer, index) => {
    ...
    return `<article class="offer-card"><div class="card-top"><div class="offer-card-price"><span class="offer-rank-index" aria-label="第 ${rank} 名">#${rank}</span><div><div class="card-price">${fmtPrice(offer.price, offer.currency)}</div>${statusMark(state(offer))}</div></div></div><div class="card-name">${esc(offer.store_name || "店铺信息待确认")}${merchantBadgeForOffer({...offer,source},merchantBadges)}${merchantXBadge({...offer,source},xProfiles)}</div>${title}${metricLine(offer)}${merchantStoreLinks({...offer,source})||claimLink(offer)}<div class="card-bottom"><span class="dim">报价时间 ${esc(quoteTimeInfo(offer).absolute ? fmtTime(quoteTimeInfo(offer).absolute) : "—")}</span>${bottomLink}</div></article>`;
  }).join("");
  return {
    total,
    page: currentPage,
...
function productPage(db, url, {analytics,req,res,merchantBadges=[],xProfiles=[],retention}={}) {
  const filters={...filtersOf(url),spec:url.searchParams.get('spec')||''};
  const {channel,framework}=filters;
  const source = url.searchParams.get("source") || "direct-shops";
  const id = url.searchParams.get("id");
  if(source==='cardnav-official'){
    const content=officialPriceContent(db,url);
    if(content)return layout('官方地区价',content,'official');
  }
  if (!id) return layout("缺少参数", `${pageIntro("缺少产品参数", "请选择一个产品后再查看报价")}<section class="surface"><div class="empty"><a href="/">返回总览</a></div></section>`);
  const snapshot = retiredCatalogSource(source) ? null : latestProducts(db, source,channel,framework);
  ...
  const rendered = offerContent(offers, { source, id, page,stale:snapshot.stale,merchantBadges,xProfiles });
  ...
  const sponsored=sponsoredContent(offers.filter(o=>quoteAvailable(source,o)),campaigns,{product:{source,product_id:id,snapshot_id:snapshot.snapshotId},viewToken:campaign=>analytics?.outbound?.viewToken(campaign)||'',recordImpression:campaign=>{if(req?.method==='GET')res?.once('finish',()=>{if(res.statusCode===200)try{analytics?.outbound?.recordImpression(req,campaign,new Date());}catch{/* Non-critical metrics. */}})}});
  const feedbackHref = "/submit?type=feedback&amp;source=" + encodeURIComponent(source) + "&amp;product=" + encodeURIComponent(id);
  const identity=directoryQuoteIdentity({source},product,product.selected_offer||{});
  const retentionKey=identity?.key;
  return layout(publicProductName(product) + " · 报价", `
    ...
    ${retentionKey?followButton(retentionKey,product.comparable?product.comparison_key:''):''}
    <section class="surface detail" id="offers"><div class="head"><div><h2>报价排行</h2><p class="note">共 ${rendered.total} 条公开报价 · 第 ${rendered.page} / ${rendered.pageCount} 页 · 展示第 ${range} 名 · 在售优先、价格升序</p></div><a class="data-feedback-link" href="${feedbackHref}">${icon("message")}反馈这页数据</a></div>${offersHtml}${rendered.pagination}</section>${sponsored}
    ${retention&&retentionKey?productReference(retention,retentionKey,{comparisonKey:product.comparable?product.comparison_key:''}):''}
    ...
    <section class="submission-shell">
      ...
      <div class="submission-panel">
        ...
        <noscript><p class="submission-status">提交需要启用 JavaScript。请启用后刷新页面；填写的信息不会通过网址发送。</p></noscript><form method="post" action="/api/submissions" class="submission-form" id="submission-form" novalidate>
          ...
          <label class="field-wide"><span class="field-label">通知邮箱<span class="optional">选填</span></span><input class="form-control" type="email" name="email" maxlength="254" autocomplete="email" placeholder="用于接收提交回执与处理结果"/><span class="field-help">仅用于本次提交的进度通知，不公开、不发送营销邮件。不填写则无法接收邮件通知。</span></label>
      ...
      (() => {
        ...
        const setMode = (nextMode) => {
          ...
          panels.forEach((panel) => {
            ...
            // Both panels share one form/radio name. The hidden panel's default
            // must not leave the active panel without a selected topic.
            if (active && !panel.querySelector('input[name="topic"]:checked')) {
              const firstTopic = panel.querySelector('input[name="topic"]');
...
function alertsPage(db, url) {
  // Reading the count first also refreshes an older read-only connection's
  // schema after the collector adds alert-group columns online.
  const visible = alertCount(db) ? db.prepare('SELECT * FROM alerts ORDER BY ts DESC,id DESC').all().filter(row=>!retiredCatalogItem(row)) : [];
  const count = visible.length;
  const pageCount=Math.max(1,Math.ceil(count/30)),page=Math.min(requestedPage(url.searchParams.get('page')),pageCount);
  const rows=visible.slice((page-1)*30,page*30);
...
async function handleSubmissionRequest(req, res, submissionsDb, create = createSubmission, {fullResult=false,successStatus=201}={}) {
  if (req.method !== "POST") {
  ...
  try {
    ...
    const result = await create(submissionsDb, payload, { clientAddress: clientAddress(req) });
    return jsonResponse(res, successStatus, fullResult ? {ok:true,...result} : { ok: true, id: result.id });
  } catch (error) {
...
async function routeRequest(req, res, { db, submissionsDb, admin, analytics, retention, xClaims, merchantUrlResolver }) {
  setSecurityHeaders(res);
  const url = new URL(req.url, "http://localhost");
  if (url.pathname==='/' && (retiredCatalogItem({key:url.searchParams.get('product')}) || ['verify','otp','relay','api'].includes(url.searchParams.get('family')) || retiredCatalogSource(url.searchParams.get('source')) || url.searchParams.get('purpose')==='api')) {
    res.statusCode=410;res.setHeader('Content-Type','text/html; charset=utf-8');res.setHeader('X-Robots-Tag','noindex, nofollow');
    res.end(layout('商品已下架','<section class="surface"><h1>该业务已下架</h1><p>相关商品不再展示。</p><a href="/">返回首页</a></section>'));return;
  }
  if(sponsorArt.has(url.pathname)){
    if(!['GET','HEAD'].includes(req.method)){res.setHeader('Allow','GET, HEAD');return jsonResponse(res,405,{error:'不支持此方法'});}
    const data=sponsorArt.get(url.pathname);res.setHeader('Content-Type','image/webp');res.setHeader('Cache-Control','public, max-age=86400');res.setHeader('Content-Length',data.length);res.end(req.method==='HEAD'?undefined:data);return;
  }
  if(url.pathname==='/assets/merchant-x.js'){
    if(!['GET','HEAD'].includes(req.method)){res.setHeader('Allow','GET, HEAD');return jsonResponse(res,405,{error:'不支持此方法'});}
    res.setHeader('Content-Type','text/javascript; charset=utf-8');res.end(req.method==='HEAD'?'':merchantXScript);return;
  }
  if (brandAssetRoute(req,res,url)) return;
  if (seoRoute(req,res,url,db,()=>loadDirectory(db))) return;
  if(await retention.route(req,res,url))return;
  if(url.pathname==='/api/sponsor-view'){
    res.setHeader('Cache-Control','no-store');
    if(req.method!=='POST'){res.setHeader('Allow','POST');return jsonResponse(res,405,{ok:false});}
    if(!sameOrigin(req)||req.headers['sec-fetch-site']==='cross-site')return jsonResponse(res,403,{ok:false});
    try{const payload=await readSubmissionJson(req);if(!payload||typeof payload!=='object'||Array.isArray(payload)||Object.keys(payload).some(k=>!['id','token'].includes(k)))return jsonResponse(res,400,{ok:false});analytics?.outbound?.recordVisible(req,payload.id,payload.token);return jsonResponse(res,200,{ok:true});}
    catch(error){return jsonResponse(res,error.status||400,{ok:false});}
  }
  ...
  if (url.pathname === '/api/merchant-applications') return handleSubmissionRequest(req,res,submissionsDb,async(_db,payload,options)=>{
    const applicant=()=>{
      const account=retention.account(req);
      if(!account)throw new MerchantApplicationError(401,'提交店铺前请先注册并登录');
      if(!retention.hasPassword(account.id))throw new MerchantApplicationError(403,'请先验证邮箱并设置账号密码');
      const profile=retention.profile(account.id);
      if(!profile)throw new MerchantApplicationError(422,'请先在“我的账号”补齐手机号和微信、TG、QQ 中的一项联系方式');
      return {accountId:account.id,email:account.email,phone:profile.phone,contact:profileContact(profile)};
    };
    const owner=applicant();
    const normalized=await normalizeMerchantSubmission(payload,{applicant:owner,resolveShopUrl:merchantUrlResolver,requestKey:owner.accountId});
    // Resolution is asynchronous; recheck the session/profile before saving.
    const current=applicant();
    if(current.accountId!==owner.accountId)throw new MerchantApplicationError(401,'登录状态已变化，请刷新后重新提交');
    return createMerchantApplication(_db,normalized,{...options,applicant:current});
  });
  if(url.pathname.startsWith('/api/shop-claims/')){
    res.setHeader('Cache-Control','no-store');res.setHeader('Referrer-Policy','no-referrer');
    const action=url.pathname.slice('/api/shop-claims/'.length);
    if(!['start','submit','status'].includes(action))return jsonResponse(res,404,{error:'接口不存在'});
    return handleSubmissionRequest(req,res,submissionsDb,(_db,input,options)=>xClaims[action](input,options),{fullResult:true,successStatus:action==='start'?201:200});
  }
  ...
  const reader=retention.account(req);
  let html;
  if (url.pathname === "/" || url.pathname === "/index.html") {
    if(normalizeShopQuery(url.searchParams.get('shop_q')))res.setHeader('X-Robots-Tag','noindex, follow');
    html = homePage(db,url,{analytics,req,res,retention,xProfiles:xClaims.profiles(),merchantBadges:submissionsDb?approvedMerchantBadges(submissionsDb):[]});
  }
  else if (url.pathname === "/product") {if(!seoProduct(db,url))res.statusCode=404;html = productPage(db, url,{analytics,req,res,retention,xProfiles:xClaims.profiles(),merchantBadges:submissionsDb?approvedMerchantBadges(submissionsDb):[]});}
  else if(url.pathname==='/official-prices')html=layout('AI 官方订阅地区价格',officialPriceContent(db,url),'official');
  else if(url.pathname==='/shop'){
    const shop=/^[a-f0-9]{24}$/.test(url.searchParams.get('id')||'')?xClaims.shop(url.searchParams.get('id')):null;
    if(!shop)res.statusCode=404;
    html=layout(shop?shop.name+' · 店铺报价与店主 X':'店铺暂未收录',shop?shopContent(shop,xClaims.profiles()):'<h1>店铺暂未收录</h1><p><a href="/submit-shop">提交店铺申请</a></p>');
  }
  else if(url.pathname==='/following')html=layout('我的关注',followingContent(),'following');
  else if(contentForPath(url.pathname))html=layout(contentForPath(url.pathname).title,contentPage(url.pathname),'guides');
  else if(['/login','/register','/reset-password','/account'].includes(url.pathname)){
    res.setHeader('X-Robots-Tag','noindex, nofollow');res.setHeader('Referrer-Policy','no-referrer');
    if(url.pathname==='/account'&&!reader){res.writeHead(303,{Location:'/login?next=%2Faccount'});return res.end();}
    if(reader&&(url.pathname==='/login'||url.pathname==='/register'&&retention.hasPassword(reader.id))){res.writeHead(303,{Location:safeAccountReturn(url.searchParams.get('next'))});return res.end();}
    const mode=url.pathname==='/reset-password'?'reset':url.pathname.slice(1);
    const nonce=randomBytes(18).toString('base64');
    res.setHeader('Content-Security-Policy',`default-src 'self'; script-src 'self' 'nonce-${nonce}'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self'; base-uri 'none'; form-action 'self'; frame-ancestors 'none'; object-src 'none'`);
    html=layout(mode==='account'?'我的账号':mode==='login'?'登录':mode==='reset'?'重置密码':'注册',mode==='account'?accountContent(reader,retention.hasPassword(reader.id),retention.profile(reader.id),safeAccountReturn(url.searchParams.get('next'))):loginContent(mode,url),'',{privateAccount:true}).replace(/<script\b/g,`<script nonce="${nonce}"`);
  }
  else if(url.pathname==='/weekly'){const content=weeklyContent(retention,url);if(!content)res.statusCode=404;html=layout('行情周报',content||'<h1>该日期的行情暂不可查看</h1>','weekly');}
  else if(url.pathname==='/unsubscribe'){res.setHeader('Referrer-Policy','no-referrer');html=layout('退订提醒',unsubscribeContent((url.searchParams.get('token')||'').slice(0,160)));}
  else if (url.pathname === "/alerts") html = alertsPage(db,url);
  else if (url.pathname === "/advertise") html=layout("广告合作",sponsorAdvertiseContent(url),"advertise");
  else if (url.pathname === "/sources") html = sourcesPage(db);
  else if (url.pathname === "/privacy") html = layout("隐私说明",`${pageIntro("隐私与访问统计","更新于2026年9月9日")}<section class="surface" style="padding:24px"><h2>第一方访问统计</h2><p>本站为了解使用情况，统计成功的公开页面访问量及访客估算。我们在服务器内短暂使用当前Cloudflare Tunnel提供的网络地址和浏览器粗分类生成HMAC去标识摘要；不保存原始IP、完整浏览器标识、完整URL、查询参数或表单信息，也不使用追踪Cookie。去标识并不等于完全匿名。</p><p>在线去重摘要保留31天，按日汇总长期保留；服务器每日备份最多14份。用于灾难恢复的加密异机历史副本长期保留，仅站长可恢复，不用于延长访问追踪。历史摘要可能继续存在于这些加密历史副本中。统计只在受保护站长后台可见。共享网络可能合并计数，换网络或浏览器可能重复计数，已知机器人过滤不能保证完全准确。</p><p>后台、API、投稿页、健康检查、预取、错误响应与带管理员会话的请求不参与统计。访客估算不能等同实际人数，服务器访问量不能等同精准真人PV。</p><h2>指南阅读统计</h2><p>订阅指南和帮助中心计入公开页面访问。为判断文章是否帮助读者继续比价，我们记录固定文章标识的每日阅读请求，以及携带该本站文章来源的后续报价页请求。仅保存按日汇总，不保存来源网址、查询参数或跨页面浏览轨迹，不添加追踪 Cookie。来源缺失可能漏计，刷新可能重复；这些请求不代表关注、购买或成交。统计前的历史不补造，汇总长期保留。</p><h2>商家跳转与广告统计</h2><p>前往商家时通过本站跳转记录出站点击，广告输出与可见展示单独统计。输出仅指服务端已输出含广告页面的请求，不代表屏幕可见曝光或真实人数；可见展示由浏览器检测广告至少 50% 面积在前台页面中持续进入屏幕 1 秒后上报活动编号和短期校验令牌。两项均按网络地址与浏览器粗分类的每日去标识摘要去重，是估算而非自然人数。用于去重的每日 HMAC 摘要明细保留31天，按日汇总长期保留；不保存原始 IP、User-Agent、来源页面 referrer 或完整查询参数，不添加追踪 Cookie。记录不代表购买或成交。</p><h2>自选关注与提醒</h2><p>游客关注、到期日期和上次查看价格保存在当前浏览器。主动确认邮箱后，关注条件与到期日期保存到私有数据库，用于跨设备同步及你自行选择的提醒。验证码在10分钟内有效；确认身份后使用30天有效的HttpOnly会话Cookie，页面验证Cookie有效1天。这些Cookie用于功能与安全，不用于跨站追踪。</p><p>仅向确认邮箱发送自选行情或续费提醒，每个邮箱每天最多一封汇总；验证码独立限流。可以暂停单项、退订全部邮件，或在我的关注中删除线上账号数据。发件服务处理必要的地址和邮件内容。关注行为的去标识明细保留31天，按日汇总长期保留；通知记录保留90天，邮件状态保留31天，验证码限流记录24小时后清除。删除后的资料仍可能存在于既有灾难恢复备份中，按备份策略保留，不用于继续发送提醒。</p><p>已知用户回访以主动使用关注功能的去标识记录估算，游客和邮箱账号分别计数。邮件链接中的退订签名只允许停止该邮箱提醒，不允许读取关注内容。</p><h2>店主 X 认领</h2><p>已收录店铺的经营者可自愿提交 X 账号并同意公开关联。认领码用于核对 X 账号控制，站方另行核对店铺归属，审核通过后才展示账号链接；联系邮箱、核验说明和内部备注不公开，不会从旧投稿的私人联系方式自动提取 X 账号。X 已关联店铺在综合排序中优先展示；价格排序只按报价，关联不代表信誉认证。</p><p>专属进度链接包含保存在浏览器中的随机查询凭据，请勿公开分享。草稿码有效7天，过期草稿会在后续申请时清除；限流摘要48小时后在后续申请时清除。已提交认领和审核记录供关联维护与争议处理使用；如需更正或删除资料，请联系站长并提供认领编号。撤销关联后停止公开展示，既有灾难恢复备份按原策略保留。分享到 X 需由本人在 X 页面确认，本站不会代发帖子。</p><h2>主动投稿</h2><p>店铺申请会记录提交账号及当时的邮箱、手机号和联系账号。删除账号不会自动删除已提交的店铺申请；如需更正或删除申请资料，请联系站长并提供申请编号。</p><p>反馈、供需与店铺申请中主动填写的信息仅供站长处理；联系方式、补充说明及内部审核记录不公开。填写有效通知邮箱后，我们会通过发件服务发送提交回执、站方回复与处理结果；发件服务会处理必要的收件地址和邮件内容，不发送营销邮件。内部备注不会进入通知。店铺申请通过后，店铺名称、公开网址与收录状态标识可在相关报价中展示。收录不代表商家信用、商品质量或交易安全保证。申请限流使用去标识摘要并在48小时后清除，不保存原始IP。请勿提交密码、API Key等敏感凭据。如需查询或删除自己主动提交的信息，请通过页脚联系方式联系站长并提供投稿编号。</p></section>`);
  else if (url.pathname === "/submit" || url.pathname === '/submit-shop' || url.pathname === '/claim-shop') {
    if(url.pathname==='/submit-shop'){
      res.setHeader('Referrer-Policy','no-referrer');
      const next=encodeURIComponent(safeAccountReturn(url.pathname+url.search));
      const destination=!reader?'/login':!retention.hasPassword(reader.id)?'/register':!retention.profile(reader.id)?'/account':null;
      if(destination){res.writeHead(303,{Location:destination+'?next='+next});return res.end();}
    }
    ...
    let claimShops=[];
    if(url.pathname==='/claim-shop'){
      res.setHeader('Cache-Control','no-store');res.setHeader('Referrer-Policy','no-referrer');
      claimShops=[...xClaims.catalog().values()];
      const selected=url.searchParams.get('shop')||'',stored=/^[a-f0-9]{24}$/.test(selected)&&xClaims.shop(selected);
      if(stored&&!claimShops.some(shop=>shop.id===selected))claimShops.unshift(stored);
    }
    html = url.pathname === '/claim-shop' ? layout('店主认领 X',claimContent(csrfToken,claimShops,url.searchParams.get('shop')||''),'submit',{csrfToken}) : url.pathname === '/submit-shop'
      ? layout('提交店铺', merchantSubmissionContent(csrfToken, {shopName:url.searchParams.get('shop')?.slice(0,100)||'',shopUrl:url.searchParams.get('url')?.slice(0,500)||'',account:{email:reader.email,...retention.profile(reader.id)}}), 'submit', {csrfToken,privateAccount:true})
  ...
  if(url.pathname==='/privacy')html=html.replace('<h2>自选关注与提醒</h2>','<h2>邮箱与密码账号</h2><p>注册时验证邮箱并设置密码，同时填写手机号及微信、TG、QQ 中一项联系账号。手机号只检查格式，不进行短信验证，也不代表已核实号码归属。联系资料保存在私有数据库，仅供本人查看修改及站方联系，不在公开页面展示。游客浏览报价无需注册，提交店铺必须登录且补齐资料。</p><p>已有邮箱关注用户可验证同一邮箱后设置密码，原有关注与退订状态保留。密码只保存带随机盐的 scrypt 哈希，不保存明文，不通过邮件发送。邮箱验证码只用于注册、首次设置或重置密码，不可单独用于登录；10分钟有效且最多尝试5次。重置密码撤销所有设备上的已有登录，需要重新用密码登录。普通用户账号与管理员、商家审核权限相互独立。</p><p>登录尝试使用带密钥的邮箱和网络标识摘要限流，记录约15分钟后清除；不保存原始IP或密码。注册本身不订阅行情或营销邮件，提醒需要自行开启。邮箱和密码表单不参与访问统计，登录与账号页面不加载第三方统计脚本。可以在我的关注中删除线上账号、联系资料、密码哈希、关注与会话；既有灾难恢复备份仍按原策略保留。</p><h2>自选关注与提醒</h2>');
  if(reader)html=html.replace('<a class="nav-link" data-reader-nav href="/login">登录 / 注册</a>','<a class="nav-link" data-reader-nav href="/account">我的账号</a>');
  if(html.includes('data-sponsor-id=')||url.pathname==='/advertise')html=html.replace('</body>','<script>'+sponsorScript+'</script></body>');
  html=decorateSeo(html,url,db,res.statusCode);
  if (req.method === "HEAD") return res.end();
  res.end(html);
}

export function createApp({ db, submissionsDb = null, analytics = null, adminOptions = {}, retentionOptions = {}, merchantXOptions = {}, merchantUrlResolver = resolveMerchantShopUrl }) {
  ...
  migrateSponsorScopes(db,analytics);
  const retention=createRetention({db,submissionsDb,...retentionOptions});
  const xClaims=createMerchantX({privateDb:submissionsDb,readDirectory:()=>loadDirectory(db),...merchantXOptions});
  const admin = createAdmin({ db, submissionsDb, analytics, retentionStore:retention.store, xClaims, backupDir: process.env.SUBMISSIONS_BACKUP_DIR, ...adminOptions });
  const server = http.createServer((req, res) => {
    ...
    routeRequest(req, res, { db, submissionsDb, admin, analytics, retention, xClaims, merchantUrlResolver }).catch((error) => {
  ...
  startMerchantWorkflow(server,{submissionsDb,db,adminOptions});
  retention.start(server);
  return server;
}
