...
function directoryQuoteRow(entry) {
...
function directoryQuotePage(category,selected,url,{analytics,req,res}={}) {
  ...
  let lastCurrency;
  const rows=shown.map(entry=>{const currency=entry.offer.currency||entry.product.currency;const heading=currency!==lastCurrency?'<h2 class="quote-currency">'+esc(currency)+' 报价</h2>':'';lastCurrency=currency;return heading+directoryQuoteRow(entry);}).join('');
...
function homePage(db, url = new URL("http://localhost"),context={}) {
  ...
  const categoryRow=item=>'<section class="directory-category" data-category="'+esc(item.key)+'"><h2 class="directory-category-title">'+esc(item.label)+'</h2><div class="directory-list">'+item.products.map(product=>{const quotes=directoryQuotes(product),references=quotes.entries.filter(entry=>entry.reference).length;return '<article class="directory-row" data-directory-product="'+esc(product.key)+'"><div><h3>'+esc(product.name)+'</h3><p class="directory-count">'+directoryCount(quotes,references,{compact:true})+'</p></div><a class="directory-action" href="'+directoryHref({family:item.key,product:product.key})+'">'+(quotes.total===references&&references?'查看参考':quotes.total?'查看店铺':'查看记录')+' '+icon('chevron')+'</a></article>';}).join('')+(item.products.length?'':'<p class="directory-empty">暂无可用报价，请稍后再来。</p>')+'</div></section>';
...
async function routeRequest(req, res, { db, submissionsDb, admin, analytics }) {
  ...
  let html;
  if (url.pathname === "/" || url.pathname === "/index.html") html = homePage(db,url,{analytics,req,res});
