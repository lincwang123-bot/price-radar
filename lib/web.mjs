...
import { projectProduct, productQuoteGroups, quoteSeries } from "./quote-policy.mjs";
...
function metricLine(offer) {
  ...
  const channelText=`<span class="dim">${esc(offer.source==="cardnav-official"?"官方渠道":offer.source==="goaihop-relay"?"服务商":channel.label)}${channel.framework?' · '+esc(channel.framework):''}</span>`;
...
function layout(title, body, active = "", options = {}) {
  ...
  if(options.filters)body=body.replace(/href="(\/product\?[^\"]*)"/g,(_,href)=>{
    ...
    for(const key of ['family','channel','q','purpose','framework','spec'])if(options.filters[key]&&options.filters[key]!=='all')target.searchParams.set(key,options.filters[key]);
  ...
  const categoryRail = active === "home" ? '<nav class="product-nav" aria-label="产品分类"><div class="product-nav-inner">'+categoryFilters.map(({key,label,platform})=>{
    const params=new URLSearchParams(options.filters||{});params.set('family',key);
  ...
  }).join('')+'</div><p class="rail-hint">左右滑动查看更多分类</p></nav>' : '';
...
<head>
  ...
  <style>${WEB_CSS}</style><style>${MARKET_CSS}</style>${analyticsScript}
</head>
<body>
...
function marketCatalog(entries,title,{business=false}={}) {
  ...
  const display=e=>({name:publicProductName(e.product),spec:e.product.comparison_label||publicSpec(e.product.spec)||e.product.product_type||"规格待确认",price:e.product.lowest_price,store:e.offer?.store_name||"",state:business?(e.list.source==="cardnav-official"?"官方参考":"服务套餐"):listingState(e.offer,e.list.stale),platform:business?(e.list.source==="cardnav-official"?"官方渠道":"服务商"):(e.channel?.label||"未确认渠道"),time:e.offer?.captured_at||e.list.fetchedAt});
...
function filtersOf(url){return {family:FAMILY_FILTERS.some(f=>f.key===url.searchParams.get('family'))?url.searchParams.get('family'):'all',channel:normalizeChannel(url.searchParams.get('channel')),framework:normalizeFramework(url.searchParams.get('framework')),q:String(url.searchParams.get('q')||'').trim().slice(0,100),purpose:PURPOSES.some(([id])=>id===url.searchParams.get('purpose'))?url.searchParams.get('purpose'):'all'};}
...
function homePage(db, url = new URL("http://localhost")) {
  const filters=filtersOf(url),{channel,framework}=filters;
  const lists=["direct-shops","priceai","ldxp-goods"].map(source=>{const list=latestProducts(db,source,channel,framework);return list?{...list,products:list.products.flatMap(p=>productQuoteGroups({...p,offers:p.offers.filter(offer=>(filters.purpose==='all'||purposeOf({...p,selected_offer:offer})===filters.purpose)&&(!filters.q||[p.name,p.spec,offer.title,offer.store_name].join(' ').toLowerCase().includes(filters.q.toLowerCase())))}))}:null;});
  const matches=e=>(filters.family==='all'||platformTone(e.product.platform)===filters.family||filters.family==='relay'&&e.list.source==='goaihop-relay')&&(filters.purpose==='all'||purposeOf(e.product)===filters.purpose)&&(!filters.q||[e.product.name,e.product.spec,e.product.selected_offer?.title,e.product.selected_offer?.store_name].join(' ').toLowerCase().includes(filters.q.toLowerCase()));
  const entries=marketEntries(lists).filter(e=>filters.family==='all'||platformTone(e.product.platform)===filters.family).sort((a,b)=>publicProductName(a.product).localeCompare(publicProductName(b.product),'zh-CN'));
  const businesses=(channel==='all'&&framework==='all'?['cardnav-official','goaihop-relay']:[]).map(source=>{const list=latestProducts(db,source);return {list,entries:list?list.products.filter(p=>p.lowest_price!=null).map(product=>({list,product,offer:null,alternatives:[]})).filter(matches):[]};});
  return layout('AI 订阅报价','<section id="comparison" class="market-intro"><div class="overview-head"><h1>AI 订阅报价</h1><p class="note">价格与库存以店铺结算页为准；参考起价不代表全部市场最低价。</p></div></section>'+channelForm(channel,filters.family,null,null,filters)+directHealthNotice(db)+'<div class="catalogs">'+marketCatalog(entries,'店铺产品')+businesses.map(b=>marketCatalog(b.entries,b.list?.source==='cardnav-official'?'官方订阅参考':'API 服务套餐',{business:true})).join('')+'</div><section class="surface empty" data-market-empty'+(entries.length||businesses.some(b=>b.entries.length)?' hidden':'')+'>当前筛选下暂无匹配报价，请尝试其他品牌、关键词或交易平台。</section><p class="note market-footer"><a href="/alerts">查看价格提醒</a></p>','home',{filters});
}
...
function offerContent(offers, { source, id, page, stale=false }) {
  ...
  const shopLink=url=>url?'<a class="offer-shop-button" data-store-risk href="'+url+'" target="_blank" rel="noopener noreferrer">'+actionLabel+' '+icon("external")+'</a>':"";
  const perPage = 10;
...
function productPage(db, url) {
  ...
  if(filters.spec)product=productQuoteGroups(product).find(p=>p.comparison_key===filters.spec)||{...product,offers:[],selected_offer:null,lowest_price:null,offer_count:0,in_stock_count:0,comparable:false};
  ...
  const related = recentAlerts(db,999).filter(alert=>{
    if(alert.source!==source||alert.product_id!==id||product.comparable===false)return false;
  ...
  return layout(publicProductName(product) + " · 报价", `
    ...
    ${channelForm(channel,filters.family==='all'?platformTone(product.platform):filters.family,source,id,filters)}${source==="direct-shops"?directHealthNotice(db):""}
...
function submissionPage(url, csrfToken) {
  ...
  const body = `
    <section class="submission-intro">
      <div class="submission-intro-copy"><span class="submission-kicker">${icon("message")}共同维护可信数据</span><h1>反馈与合作</h1><p>纠正公开数据，或提交可核验的供需信息。先选择类型，再补充必要事实，不需要从头写一大段说明。</p></div>
