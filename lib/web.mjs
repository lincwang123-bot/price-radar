...
import {createMerchantX} from './merchant-x.mjs';
...
async function routeRequest(req, res, { db, submissionsDb, admin, analytics, retention, xClaims }) {
  ...
  let html;
  if (url.pathname === "/" || url.pathname === "/index.html") html = homePage(db,url,{analytics,req,res,retention,xProfiles:xClaims.profiles(),merchantBadges:submissionsDb?approvedMerchantBadges(submissionsDb):[]});
