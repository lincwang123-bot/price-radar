...
<head>
  ...
  <style>${WEB_CSS}</style><style>${MARKET_CSS}</style><style>${DIRECTORY_CSS}</style>${analyticsScript}
</head>
<body>
...
function homePage(db, url = new URL("http://localhost")) {
  ...
  const categoryRow=item=>'<a class="directory-row" data-category="'+esc(item.key)+'" href="/?family='+encodeURIComponent(item.key)+'"><h2>'+esc(item.label)+'</h2><span class="directory-action">查看产品 '+icon('chevron')+'</span></a>';
  let title='AI 订阅报价',content;
  if(!category){
    content='<div class="directory-intro"><h1>AI 订阅报价</h1><p>选择分类，找到你要的产品。</p></div><div class="directory-list">'+directory.filter(item=>FAMILY_FILTERS.some(f=>f.key===item.key&&!f.secondary)).map(categoryRow).join('')+'</div><details class="directory-more"><summary>更多分类</summary><div class="directory-list">'+directory.filter(item=>FAMILY_FILTERS.some(f=>f.key===item.key&&f.secondary)).map(categoryRow).join('')+'</div></details>';
  }else if(selected){
    title=selected.name+' · 选择规格';
    const variants=selected.variants.map(entry=>{
      const product=entry.product,offer=entry.offer;
      const spec=product.comparison_label||offer?.comparison_label||publicSpec(product.spec)||product.name;
      const platform=entry.list.source==='cardnav-official'?'官方参考':entry.list.source==='goaihop-relay'?'API 服务商':entry.channel?.label||offerChannel(offer||{}).label;
      const href=productHref(entry.list.source,product.product_id)+(SHOP_SOURCES.has(entry.list.source)&&product.comparison_key?'&amp;spec='+encodeURIComponent(product.comparison_key):'');
      return '<article class="directory-row directory-variant" data-product-variant><div><h2>'+esc(spec)+'</h2><p>'+esc([platform,offer?.store_name].filter(Boolean).join(' · '))+'</p></div><span class="money">'+fmtPrice(offer?.price??product.lowest_price,offer?.currency||product.currency)+'</span><a class="directory-action" href="'+href+'">查看店铺报价 '+icon('chevron')+'</a></article>';
    }).join('');
    const unavailable=(selected.references||[]).filter(entry=>!selected.variants.some(variant=>variant.list.source===entry.list.source&&variant.product.product_id===entry.product.product_id)).map(entry=>'<p class="note"><a href="'+productHref(entry.list.source,entry.product.product_id)+'">查看 '+esc(entry.product.name||selected.name)+' 的报价记录</a></p>').join('');
    content='<a class="breadcrumb" href="/?family='+encodeURIComponent(category.key)+'">'+icon('back')+'返回 '+esc(category.label)+'</a><div class="directory-intro"><h1>'+esc(selected.name)+'</h1><p>选择规格，再查看店铺报价。价格与库存以店铺结算页为准。</p></div><div class="directory-list">'+(variants||'<p class="directory-empty">暂无可用报价，请稍后再来。</p>')+'</div>'+unavailable;
  }else{
    title=category.label+' · 产品';
    content='<a class="breadcrumb" href="/">'+icon('back')+'返回总览</a><div class="directory-intro"><h1>'+esc(category.label)+'</h1><p>选择产品，查看规格和报价。</p></div><div class="directory-list">'+category.products.map(product=>'<article class="directory-row" data-directory-product="'+esc(product.key)+'"><h2>'+esc(product.name)+'</h2><a class="directory-action" href="/?family='+encodeURIComponent(category.key)+'&amp;product='+encodeURIComponent(product.key)+'">查看报价 '+icon('chevron')+'</a></article>').join('')+(category.products.length?'':'<p class="directory-empty">暂无可用报价，请稍后再来。</p>')+'</div>';
  }
