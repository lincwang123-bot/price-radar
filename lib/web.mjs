...
function directoryRiskNotice(family = 'all') {
  const chatgpt = ['all','chatgpt'].includes(family)
    ? '<p class="chatgpt-price-notice"><strong>ChatGPT 低价引流提醒</strong>部分 Pro 20x（20倍）低价仅限特定账号或续费条件。警惕添加微信、Telegram 等私聊后，被要求补差价或改买高价套餐。付款前确认可充条件、最终总价、订阅时长和退款规则；临时加价时请先暂停交易，并<a href="/submit?type=feedback">反馈问题报价</a>。</p>'
    : '';
  return '<aside class="directory-risk-note" role="note" aria-label="交易风险提醒">'+icon('warning')+'<div>'+chatgpt+'<p><strong>交易风险提醒</strong>本站仅提供信息导航，不参与交易或售后；收录不代表交易安全保证。下单前核实商品规格与售后条款，警惕低价诱导和私下转账。</p></div></aside>';
}
function loadDirectory(db) {
  const lists=["direct-shops","priceai","ldxp-goods","cardnav-official"].map(source=>{
...
function homePage(db, url = new URL("http://localhost"),context={}) {
  ...
  if(!category){
    ...
    content='<a class="breadcrumb" href="/">'+icon('back')+'返回全部产品</a><div class="directory-intro"><h1>'+platformMark(category.key)+esc(category.label)+'</h1><p>点击产品，查看各家店铺报价。</p></div>'+directoryRiskNotice(category.key)+categoryRow(category);
  }
