...
<head>
  ...
  <style>${WEB_CSS}${MERCHANT_BADGE_CSS}${MERCHANT_X_CSS}</style><style>${MARKET_CSS}</style><style>${DIRECTORY_CSS}</style><style>${HOME_PRICE_CSS}</style><style>${GUIDE_CSS}</style>${analyticsScript}
</head>
<body>
...
function directoryQuotePage(category,selected,url,{analytics,req,res,merchantBadges=[],xProfiles=[]}={}) {
  ...
  for(const entry of selected.quoteEntries||selected.entries){if(entry.reference || selected.deliveryEnabled&&offerDelivery(entry.offer).kind!==state.delivery || state.channel!=='all'&&entry.channel?.id!==state.channel || state.currency&&(entry.offer.currency||entry.product.currency)!==state.currency || state.spec&&entry.offer.comparison_key!==state.spec)continue;const key=JSON.stringify([entry.list.source,entry.product.product_id]);if(!groups.has(key))groups.set(key,[]);groups.get(key).push(entry);}
  let sponsored='';
