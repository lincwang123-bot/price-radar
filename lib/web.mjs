...
import {contentForPath,contentPage,productBuyingNote} from './guides.mjs';
...
function directoryQuotePage(category,selected,url,{db,analytics,req,res,merchantBadges=[],xProfiles=[]}={}) {
  ...
  return '<a class="breadcrumb" href="/?family='+encodeURIComponent(category.key)+'">'+icon('back')+'返回 '+esc(category.label)+'</a><div class="directory-intro"><h1>'+platformMark(category.key)+esc(selected.name)+'</h1><p>'+directoryCount(result,references.length)+(shops.length?' · '+sortLabel:'')+'</p><p>不同周期、交付方式的总价不可直接比较；价格与库存以店铺结算页为准。</p>'+(result.currencies.length>1?'<p>不同币种分别排序，不做汇率换算。</p>':'')+'</div>'+productBuyingNote(selected.key)+controls+(!shops.length&&references.length?'':'<div class="directory-quotes">'+(rows||'<p class="directory-empty">暂无可用报价，请调整筛选或稍后再来。</p>')+'</div><p class="note">第 '+page+' / '+pageCount+' 页 · 每页 20 条店铺报价</p>'+pagination)+sponsored+(references.length?'<section class="directory-references"><h2>参考报价</h2><p class="note">以下为官方订阅参考，不计入店铺报价。</p>'+references.map(entry=>directoryQuoteRow(entry)).join('')+'</section>':'')+unavailable;
}
...
function productPage(db, url, {analytics,req,res,merchantBadges=[],xProfiles=[],retention}={}) {
  ...
  return layout(publicProductName(product) + " · 报价", `
    ...
    ${productBuyingNote(retentionKey)}
