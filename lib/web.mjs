...
function directoryDeliveryLinks(product, family, quotes) {
  const render = group => {
    ...
    return '<a class="directory-delivery-card" data-delivery-link="'+group.kind+'" href="'+directoryHref({family,product:product.key,delivery:group.kind})+'"><span class="directory-delivery-label">'+esc(group.label)+'</span>'+price+'<span class="directory-delivery-cta">查看店铺 ›</span></a>';
  };
  const primary=quotes.deliveries.filter(group=>['recharge','account'].includes(group.kind));
  const other=quotes.deliveries.filter(group=>!['recharge','account'].includes(group.kind));
  return '<div class="directory-delivery-links">'+primary.map(render).join('')+(other.length?'<details class="directory-delivery-more"><summary>其他交付 · '+other.reduce((n,g)=>n+g.count,0)+' 条</summary><div>'+other.map(render).join('')+'</div></details>':'')+'</div>';
}
