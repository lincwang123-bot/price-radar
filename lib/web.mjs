...
function directoryQuotePage(category,selected,url,{analytics,req,res,merchantBadges=[],xProfiles=[]}={}) {
  ...
  const state={family:category.key,product:selected.key,delivery:activeDelivery,sort:['desc','price_desc'].includes(url.searchParams.get('sort'))?'price_desc':'price_asc',channel:normalizeChannel(url.searchParams.get('channel')),spec:url.searchParams.get('spec')||'',currency:url.searchParams.get('currency')||''};
  ...
  const controls=deliveryControls+'<div class="quote-controls"><div class="spec-selector" aria-label="交易平台">'+CHANNELS.map(channel=>choice('channel',channel.id,channel.label)).join('')+'</div>'+((result.specs.length>1||result.currencies.length>1||state.spec||state.currency)?'<details class="detail-filters"'+(state.spec||state.currency?' open':'')+'><summary>规格与币种筛选</summary><div class="spec-selector" aria-label="规格筛选">'+choice('spec','','全部规格')+result.specs.map(spec=>choice('spec',spec.key,spec.label+' · '+spec.count)).join('')+'</div><div class="spec-selector" aria-label="币种筛选">'+choice('currency','','全部币种')+result.currencies.map(currency=>choice('currency',currency,currency)).join('')+'</div></details>':'')+'</div>';
  let lastCurrency;
  ...
  return '<a class="breadcrumb" href="/?family='+encodeURIComponent(category.key)+'">'+icon('back')+'返回 '+esc(category.label)+'</a><div class="directory-intro"><h1>'+platformMark(category.key)+esc(selected.name)+'</h1><p>'+directoryCount(result,references.length)+(shops.length?' · 价格由'+(state.sort==='price_desc'?'高到低':'低到高'):'')+'</p><p>不同周期、交付方式的总价不可直接比较；价格与库存以店铺结算页为准。</p>'+(result.currencies.length>1?'<p>不同币种分别排序，不做汇率换算。</p>':'')+'</div>'+controls+(!shops.length&&references.length?'':'<div class="directory-quotes">'+(rows||'<p class="directory-empty">暂无可用报价，请调整筛选或稍后再来。</p>')+'</div><p class="note">第 '+page+' / '+pageCount+' 页 · 每页 20 条店铺报价</p>'+pagination)+sponsored+(references.length?'<section class="directory-references"><h2>参考报价</h2><p class="note">以下为官方订阅或 API 服务参考，不计入店铺报价。</p>'+references.map(entry=>directoryQuoteRow(entry)).join('')+'</section>':'')+unavailable;
}
