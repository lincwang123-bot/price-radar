...
function productPage(db, url, {analytics,req,res,merchantBadges=[],retention}={}) {
...
async function routeRequest(req, res, { db, submissionsDb, admin, analytics, retention }) {
  ...
  let html;
  if (url.pathname === "/" || url.pathname === "/index.html") html = homePage(db,url,{analytics,req,res,retention,merchantBadges:submissionsDb?approvedMerchantBadges(submissionsDb):[]});
  else if (url.pathname === "/product") {if(!seoProduct(db,url))res.statusCode=404;html = productPage(db, url,{analytics,req,res,retention,merchantBadges:submissionsDb?approvedMerchantBadges(submissionsDb):[]});}
...
export function createApp({ db, submissionsDb = null, analytics = null, adminOptions = {}, retentionOptions = {} }) {
  ...
  const admin = createAdmin({ db, submissionsDb, analytics, retentionStore:retention.store, backupDir: process.env.SUBMISSIONS_BACKUP_DIR, ...adminOptions });
  const server = http.createServer((req, res) => {
    ...
    routeRequest(req, res, { db, submissionsDb, admin, analytics, retention }).catch((error) => {
