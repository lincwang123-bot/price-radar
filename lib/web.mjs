...
async function routeRequest(req, res, { db, submissionsDb, admin, analytics, retention, xClaims }) {
  ...
  if (url.pathname === "/" || url.pathname === "/index.html") {
    ...
    if(reader&&(url.pathname==='/login'||url.pathname==='/register'&&retention.hasPassword(reader.id))){res.writeHead(303,{Location:'/account'});return res.end();}
    ...
    html=layout(mode==='account'?'我的账号':mode==='login'?'登录':mode==='reset'?'重置密码':'注册',mode==='account'?accountContent(reader,retention.hasPassword(reader.id)):loginContent(mode,url),'',{privateAccount:true}).replace(/<script\b/g,`<script nonce="${nonce}"`);
  }
