...
function metricLine(offer) {
  ...
  const channelText=`<span class="dim">${esc(offer.source==="cardnav-official"?"官方渠道":offer.source==="goaihop-relay"?"服务商":channel.label)}${channel.framework?' · '+esc(channel.framework):''}</span><span class="quote-provenance" title="${esc(time.absolute||'')}">${esc(quoteSourceLabel(offer))} · ${esc(time.relative)}${time.staleLabel?' · '+esc(time.staleLabel):''}</span>`;
...
function productPage(db, url, {analytics,req,res}={}) {
...
async function routeRequest(req, res, { db, submissionsDb, admin, analytics }) {
  ...
  else if (url.pathname === "/product") {if(!seoProduct(db,url))res.statusCode=404;html = productPage(db, url,{analytics,req,res});}
