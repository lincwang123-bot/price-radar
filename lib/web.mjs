...
function directShopPriceSeriesRows(db, productId) {
  ...
  for (const row of rows) {
    ...
    if (!["in_stock", "available", "online"].includes(String(row.status ?? "").toLowerCase())) continue;
  ...
  const body = `
    ...
    <section class="submission-shell">
      ...
      <div class="submission-panel">
        <div class="submission-panel-head"><div><h2 data-submission-title>${mode === "feedback" ? "反馈一处问题" : "提交合作信息"}</h2><p data-submission-description>${mode === "feedback" ? "选好问题类型后，告诉我们是哪项数据需要复核。" : "说明供需方向、规模与保障边界，便于快速判断是否匹配。"}</p></div><span class="submission-step">人工审核</span></div>
        <form class="submission-form" id="submission-form" novalidate>
          ...
          <div data-kind-panel="cooperation"${cooperationHidden}>
            ...
            <section class="form-section"><div class="form-grid">
              <label class="field-wide"><span class="field-label">提交标题</span><input class="form-control" name="subject" minlength="4" maxlength="120" required placeholder="例如：Claude 企业订阅月度稳定供给"${cooperationDisabled}/></label>
              <label><span class="field-label">产品方向</span><select class="form-control" name="productArea" required${cooperationDisabled}><option value="chatgpt">ChatGPT</option><option value="claude">Claude</option><option value="gemini">Gemini</option><option value="grok_x">Grok / X</option><option value="api_relay">API / 中转</option><option value="mail_verify">邮箱 / 接码</option><option value="other">其他</option></select></label>
              <label><span class="field-label">合作规模</span><select class="form-control" name="scale" required${cooperationDisabled}><option value="trial">先小额测试</option><option value="small">稳定小批量</option><option value="monthly">月度批量</option><option value="large">大额长期</option><option value="negotiable">待沟通</option></select></label>
              <label><span class="field-label">保障方式</span><select class="form-control" name="assurance" required${cooperationDisabled}><option value="full_warranty">明确质保</option><option value="subscription_cover">掉订阅保障</option><option value="activation_only">仅保证激活可用</option><option value="conditional">有条件保障</option><option value="none">无质保</option><option value="negotiable">可协商</option></select></label>
              <label><span class="field-label">结算方式</span><select class="form-control" name="settlement" required${cooperationDisabled}><option value="cny">人民币</option><option value="usdt">USDT</option><option value="both">人民币或 USDT</option><option value="negotiable">可协商</option></select></label>
              <label class="field-wide"><span class="field-label">公开证明或介绍链接<span class="optional">选填</span></span><input class="form-control" type="url" name="contextUrl" maxlength="500" placeholder="https:// 开头的店铺、产品或企业介绍页"${cooperationDisabled}/><span class="field-help">本站只保存链接供人工核验，不会自动抓取其中内容。</span></label>
              <label class="field-wide"><span class="field-label">合作说明</span><textarea class="form-control form-textarea" name="details" minlength="10" maxlength="1500" required placeholder="请写明具体产品、交付方式、预计量级、报价口径、测试方式和保障范围。不要提交账号密码、卡密或 API Key。"${cooperationDisabled}></textarea></label>
          ...
          <div class="submission-actions"><p class="submission-privacy">为防止滥用，本站会使用带密钥的短期限流标识，不保存原始 IP 或浏览器标识。联系方式只用于核实和后续沟通。</p><button class="submit-button" type="submit"><span data-submit-label>提交审核</span>${icon("send")}</button></div>
        </form>
    ...
    <script>
      (() => {
        ...
        const setMode = (nextMode) => {
          ...
          title.textContent = isFeedback ? "反馈一处问题" : "提交合作信息";
          description.textContent = isFeedback ? "选好问题类型后，告诉我们是哪项数据需要复核。" : "说明供需方向、规模与保障边界，便于快速判断是否匹配。";
          status.textContent = "";
...
async function routeRequest(req, res, { db, submissionsDb }) {
...
export function createApp({ db, submissionsDb = null }) {
  const server = http.createServer((req, res) => {
    routeRequest(req, res, { db, submissionsDb }).catch((error) => {
