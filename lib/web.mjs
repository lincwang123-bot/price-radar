...
function latestProducts(db, source, channel = "all") {
  ...
  if(channel!=="all"||SHOP_SOURCES.has(source))products=products.flatMap(p=>{const offers=filterChannel(offersOf(db,source,latest.snapshot_id,p.product_id),channel).filter(o=>Number.isFinite(Number(o.price))&&Number(o.price)>0);if(!offers.length&&channel!=="all")return[];const available=offers.filter(o=>['in_stock','available','online','low_stock'].includes(o.status)&&o.stock_count!==0);return[{...p,public_sample_only:source==='priceai',lowest_price:available.length?Math.min(...available.map(o=>Number(o.price))):null,offer_count:offers.length,visible_offer_count:offers.length,in_stock_count:available.length}];});
...
function offersOf(db, source, snapshotId, productId) {
  ...
  ).all(source, snapshotId, productId).filter(o=>publicOfferAllowed(source,o));
}
...
function productPage(db, url) {
  ...
  const sourceLimitNotice = source === "priceai" && offers.length > 0 && hasUnpublishedOfferDetails(product)
    ? `<div class="offer-source-note" role="note"><strong>数据范围</strong><span>PriceAI 上游只公开部分 Top 报价，本页展示经过售罄与明确无质保过滤后的 ${offers.length} 条公开样本，并非其全站可用数量。${directMatch ? "本站已将匹配的原始店铺直采结果单独列出。" : ""}</span>${directCta}</div>`
    : "";
