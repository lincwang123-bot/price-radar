...
function latestProducts(db, source, channel = "all") {
  ...
  if(channel!=="all")products=products.flatMap(p=>{const offers=filterChannel(offersOf(db,source,latest.snapshot_id,p.product_id),channel).filter(o=>Number.isFinite(Number(o.price))&&Number(o.price)>0);if(!offers.length)return[];const available=offers.filter(o=>['in_stock','available','online','low_stock'].includes(o.status)&&o.stock_count!==0);return[{...p,lowest_price:Math.min(...(available.length?available:offers).map(o=>Number(o.price))),offer_count:offers.length,visible_offer_count:offers.length,in_stock_count:available.length}];});
...
function statusMark(value) {
  ...
  else if (raw === "low_stock") { label = "库存紧张"; tone = "available"; }
...
function metricLine(offer) {
  ...
  const channelText=`<span class="dim">${esc(channel.label)}${channel.framework?' · '+esc(channel.framework):''}</span>`;
  ...
  const categoryScript = active === "home" ? `<script>
    (() => {
      ...
      const setFamily = (family) => {
        ...
        if (title) title.textContent = hasComparison ? originalTitle : selected.lastElementChild.textContent + " 公开报价";
        if (description) description.textContent = hasComparison ? originalDescription : "按数据来源查看产品与报价，具体规格、库存和保障请以店铺页面为准。";
      };
...
<body>
  <header class="app-header"><div class="header-inner"><div class="header-main">
    ...
    <nav class="app-nav" aria-label="主导航">${nav("/", "home", "总览")}${nav("/alerts", "alerts", "提醒")}${nav("/sources", "sources", "数据源")}${nav("/submit", "submit", "提交")}</nav>
...
function homePage(db, url = new URL("http://localhost")) {
  ...
  const priceai = latestProducts(db, "priceai",channel);
  const direct = latestProducts(db, "direct-shops",channel);
  const cardnav = latestProducts(db, "cardnav-official",channel);
  const ldxp = latestProducts(db, "ldxp-goods",channel);
  const goaihop = latestProducts(db, "goaihop-relay",channel);
  const overview = overviewContent(db, priceai, direct, cardnav, ldxp);
  const family = familyContent(db, priceai, direct, cardnav, ldxp);
  const availableFamilies = new Set(FAMILIES.map(({ platform }) => platformTone(platform)));
  for (const list of [direct, priceai, cardnav, ldxp, goaihop]) {
    if (!list) continue;
    for (const group of sourceProductGroups(list, list.products.filter((product) => Number(product.lowest_price) > 0))) {
      if (group.items.length) availableFamilies.add(group.tone);
    }
  }
  return layout("总览", `
    ${channelForm(channel,url.searchParams.get("family")||"all")}
    ${directHealthNotice(db)}
  ...
  `, "home", { availableFamilies, channel });
}

function channelForm(channel,family,source=null,id=null){return `<form method="get" action="${source?"/product":"/"}" class="surface" style="padding:12px;margin-bottom:16px"><label for="channel-filter">交易渠道 </label><select id="channel-filter" name="channel" style="font:inherit;padding:8px">${CHANNELS.map(c=>`<option value="${c.id}"${c.id===channel?" selected":""}>${c.label}</option>`).join("")}</select><input type="hidden" name="family" data-channel-family value="${esc(family)}">${source?`<input type="hidden" name="source" value="${esc(source)}"><input type="hidden" name="id" value="${esc(id)}">`:""}<button type="submit" style="font:inherit;padding:8px;margin-left:8px">筛选</button><p class="note">渠道按已核实原站域名归类；软件框架不代表交易托管或保障。</p></form>`;}
...
function channelSeries(db,source,id,channel){
 ...
 const points=new Map();for(const o of filterChannel(rows,channel)){if(o.stock_count===0||!["in_stock","available","online","low_stock"].includes(o.status))continue;if(source==="direct-shops"&&directOfferExclusionReason({...o,stockCount:o.stock_count}))continue;const price=Number(o.price);if(!(price>0))continue;const old=points.get(o.snapshot_id);if(!old||price<old.lowest_price)points.set(o.snapshot_id,{fetched_at:o.fetched_at,lowest_price:price});}return [...points.values()];
}
...
function productPage(db, url) {
  ...
  if(!product&&channel!=="all"){const original=latestProducts(db,source)?.products.find(item=>item.product_id===id);if(original)product={...original,lowest_price:null,offer_count:0,visible_offer_count:0,in_stock_count:0};}
...
export function createApp({ db, submissionsDb = null, adminOptions = {} }) {
  const admin = createAdmin({ db, submissionsDb, backupDir: process.env.SUBMISSIONS_BACKUP_DIR, ...adminOptions });
...
export function startWeb({ db, submissionsDb = null, host = "127.0.0.1", port = 8090 }) {
  const app = createApp({ db, submissionsDb });
