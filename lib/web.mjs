...
function layout(title, body, active = "", options = {}) {
  ...
  const categoryScript = active === "home" ? `<script>
    (() => {
      ...
      if (!comparison || !buttons.length) return;
...
function homePage(db, url = new URL("http://localhost")) {
  ...
  const requestedFamily=url.searchParams.get("family");if(FAMILY_FILTERS.some(f=>f.key===requestedFamily))availableFamilies.add(requestedFamily);
...
function channelForm(channel,family,source=null,id=null){return `<form method="get" action="${source?"/product":"/"}" class="surface channel-form"><span class="channel-label">交易平台</span><div class="channel-options" role="group" aria-label="交易平台">${CHANNELS.map(c=>`<button class="channel-pill${c.id===channel?" selected":""}" type="submit" name="channel" value="${c.id}" aria-pressed="${c.id===channel}">${c.label}</button>`).join("")}</div><input type="hidden" name="family" data-channel-family value="${esc(family)}">${source?`<input type="hidden" name="source" value="${esc(source)}"><input type="hidden" name="id" value="${esc(id)}">`:""}<p class="note">按交易平台筛选；库存与价格以店铺页面为准。</p></form>`;}
