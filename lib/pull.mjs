...
import { recordCoverageDaily } from './source-coverage.mjs';
import { processMerchantPreflights } from './merchant-preflight-worker.mjs';
import { filterCatalogSnapshot, retiredCatalogSource } from './catalog-policy.mjs';
...
export async function runPull(opts, onlySources = []) {
  ...
  const results = [];
  const sources = Object.entries(config.sources ?? {}).filter(([id, s]) => s.enabled && !retiredCatalogSource(id));

  // Manual preflight requests must not wait for a particular source's 30m cache.
  try {
    const preflight = await processMerchantPreflights(opts);
    if (!preflight.valid) log('[merchant-preflight] 测试队列无效，本轮未访问店铺。');
    else if (preflight.processed) log(`[merchant-preflight] 已完成 ${preflight.processed} 项接入检测。`);
  } catch { log('[merchant-preflight] 本轮测试处理失败，常规采集继续。'); }
  ...
  for (const [id, srcCfg] of sources) {
    ...
    try {
      ...
      let stored = null;
      if (outcome.snapshot) {
        stored = storeSnapshot(db, filterCatalogSnapshot(outcome.snapshot));
      } else {
        // 适配器未产出快照（旧指针语义的兜底）
  ...
  try {
    const assessment=recordCoverageDaily(db,path.join(dataDir,'source-coverage-ledger.json'));
    if(assessment.written)log(`[coverage] ${assessment.result.status}; ${assessment.gate.recommendation}`);
  } catch { log('[coverage] 本轮验收记录失败；采集结果已保留。'); }
  return results;
}
