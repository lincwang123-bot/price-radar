...
export async function runPull(opts, onlySources = []) {
  ...
  for (const [id, srcCfg] of sources) {
    ...
    try {
      ...
      if (!outcome.skipped) metaSet(db,`health:${id}`,JSON.stringify({source:id,checkedAt:new Date().toISOString(),status:outcome.snapshot?.stale?"stale":"ok",snapshotId:after}));
      results.push({
        source: id,
        ok: true,
        ms: Date.now() - started,
