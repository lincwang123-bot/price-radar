...
export function copyMessageContent(message, id, {label='发给店主的文案',buttonLabel='复制文案',hint='可直接选中文案复制；请核对后自行发送。'}={}) {
  return `<label class="field" for="${esc(id)}"><span>${esc(label)}</span></label><textarea class="merchant-copy-text" id="${esc(id)}" readonly rows="6">${esc(message)}</textarea><div class="actions"><button type="button" class="secondary" data-copy-target="${esc(id)}" data-copy-status="${esc(id)}-status">${esc(buttonLabel)}</button><span class="muted" id="${esc(id)}-status" role="status" aria-live="polite">${esc(hint)}</span></div>`;
}
