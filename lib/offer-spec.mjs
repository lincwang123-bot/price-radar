...
export function offerSpec(offer, product = {}) {
  ...
  const subscription = /chatgpt|gpt|claude|gemini|grok|premium|cursor|perplexity|notion/i.test(`${title} ${product.product_id || product.id || ''}`) && !/api|额度|积分|接码|邮箱|辅助服务|activation-service|claim-link|verification-service|email-accounts|(?:^|-)free(?:-|$)/.test(`${product.product_type || product.productType || ''} ${product.product_id || product.id || ''}`) && !freeAccount;
  ...
  const tier = /(?:20\s*x|x\s*20)/.test(title)?'20x':/(?:5\s*x|x\s*5)/.test(title)?'5x':/premium\s*(?:\+|plus)/.test(title)?'premium+':/pro\s*(?:\+|plus)/.test(title)?'pro+':/ultra/.test(title)?'ultra':/heavy/.test(title)?'heavy':/\bmax\b/.test(title)?'max':/\bpro\b/.test(title)?'pro':/plus/.test(title)?'plus':/premium/.test(title)?'premium':'';
