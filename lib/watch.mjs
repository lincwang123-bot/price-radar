...
// Optional constraints: groupId = exact public comparison_key, term = e.g. 1m/12m,
// currency = e.g. CNY. Legacy rules without constraints still evaluate each group
// separately; no product-level low is compared across terms, delivery, or currency.
import {
  recentSnapshots, productsOfSnapshot, metaGet, metaSet,
} from "./db.mjs";
import { projectProduct, productQuoteGroups, sourceHealth, quoteAvailable } from './quote-policy.mjs';

import {retiredCatalogSource} from './catalog-policy.mjs';
const EPS = 1e-6;
...
function fpOf(o) {
  return o?.url || o?.offer_id || o?.offerId || null;
}
...
function evalRule(db, rule, sourceId, snapList, snapById) {
  ...
  if(rule.kind==='offer_gone')for(const p of prevProducts.values())if(!products.some(row=>row.product_id===p.product_id))products.push(p);
  for (const prod of products) {
    if (rule.product && rule.product !== "*" && prod.product_id !== rule.product) continue;
    const projected=projectProduct(db,sourceId,latest,prod);
    const currentGroups=productQuoteGroups(projected);
    if(rule.kind==='offer_gone'&&prev){const old=prevProducts.get(prod.product_id);if(old)for(const g of productQuoteGroups(projectProduct(db,sourceId,prev,old,{historical:true})))if(!currentGroups.some(c=>c.comparison_key===g.comparison_key))currentGroups.push({...g,offers:[],lowest_price:null,stale:false});}
    for(const current of currentGroups) {
    if(!current.comparable||current.stale||current.offers.some(o=>o.quote_stale)||!current.currency)continue;
    if(rule.groupId&&rule.groupId!==current.comparison_key)continue;
    if(rule.term&&rule.term!==current.comparison_key.split(':')[0])continue;
    if(rule.currency&&rule.currency!==current.currency)continue;
    const key = JSON.stringify(['groups-v2',rule,sourceId,prod.product_id,current.comparison_key]);
    const offers = current.offers.filter(o=>quoteAvailable(sourceId,o));
    ...
    const previous = prevProd ? productQuoteGroups(projectProduct(db,sourceId,prev,prevProd,{historical:true})).find(g=>g.comparison_key===current.comparison_key) : null;
    if(previous && (!previous.comparable || previous.stale || previous.offers.some(o=>o.quote_stale) || previous.comparison_key !== current.comparison_key)) continue;
    const prevLow = previous?.lowest_price ?? null;
    const prevCheapest = cheapestOffer((previous?.offers ?? []).filter(o=>quoteAvailable(sourceId,o)));
    const name = (current.name || prod.name || prod.product_id)+'（'+(current.comparison_label||current.currency)+'）';
    ...
    const push = (kind, message, productId = prod.product_id) =>
      alerts.push({
        ...
        ruleId: rule.id,
        kind,
        groupId: current.comparison_key,
        groupLabel: current.comparison_label,
        message,
      });
    ...
    if (rule.kind === "min_below" && curLow != null && rule.threshold != null) {
      ...
      if (below) {
        ...
        if (crossedDown || newLow) {
          push("min_below",
            `${name} 最低价 ${fmtPrice(curLow,current.currency)} 已跌至阈值 ${fmtPrice(rule.threshold,current.currency)} 以下` +
            (prevLow != null && curLow < prevLow - EPS ? `（上轮 ${fmtPrice(prevLow,current.currency)}）` : ""));
        }
        st.wasBelow = true;
    ...
    if (rule.kind === "drop_pct" && curLow != null && rule.pct != null) {
      ...
      for (let i = 1; i < snaps.length; i++) {
        ...
        const point = row ? productQuoteGroups(projectProduct(db,sourceId,snaps[i],row,{historical:true})).find(g=>g.comparison_key===current.comparison_key) : null;
        if (point?.comparable && !point.offers.some(o=>o.quote_stale) && point.comparison_key === current.comparison_key && point.lowest_price != null) windowLows.push(point.lowest_price);
      }
      ...
      if (high != null && high > EPS) {
        ...
        if (dropPct >= rule.pct - EPS) {
          ...
          if (newly || deeper) {
            push("drop_pct",
              `${name} 窗口高点 ${fmtPrice(high,current.currency)} → 现最低 ${fmtPrice(curLow,current.currency)}，跌幅 ${dropPct.toFixed(1)}% ≥ ${rule.pct}%`);
          }
          st.active = true;
      ...
      if (curCheapest && prevCheapest) {
        ...
        if (a && b && a !== b) {
          ...
          push("cheapest_changed",
            ...
            `${fmtPrice(prevPrice,current.currency)} → ${curCheapest.store_name || curCheapest.source_name || "-"} ` +
            `${fmtPrice(curPrice,current.currency)} ${delta}  ${curCheapest.url || ""}`);
        }
      }
    }

    if (rule.kind === "offer_gone" && Number(latest.stale)!==1 && !current.offers.some(o=>o.quote_stale)) {
      const st = stateGet(db, key);
      ...
      if (goneNow && (!st.gone || st.goneFingerprint !== prevFp)) {
        push("offer_gone",
          `${name} 上次最低报价 ${fmtPrice(prevLow ?? prevCheapest?.price,current.currency)}` +
          (vanished ? `（${prevCheapest?.url || prevCheapest?.source_name || "?"}）已消失/无货` : " 全场无货") +
          (curLow != null ? `；现最低 ${fmtPrice(curLow,current.currency)}` : ""));
      }
      st.gone = goneNow;
      st.goneFingerprint = prevFp;
      stateSet(db, key, st);
    }
    }
  }
 ...
 */
function runWatchInner(db, watchCfg) {
  if(watchCfg?.enabled===false)return [];
  const alerts = [];
  const columns=new Set(db.prepare('PRAGMA table_info(alerts)').all().map(c=>c.name));
  for(const column of ['group_id','group_label'])if(!columns.has(column))db.exec('ALTER TABLE alerts ADD COLUMN '+column+' TEXT');
  ...
  for (const rule of rules) {
    const sourceId = rule.source ?? "direct-shops";
    if(retiredCatalogSource(sourceId))continue;
  ...
  for (const [sourceId, sourceRules] of bySource) {
    ...
    const signature=JSON.stringify(['groups-v2',sourceRules]);
    if (wm === latestId && metaGet(db,wmKey+'_rules')===signature) {
    ...
    metaSet(db,wmKey+'_rules',signature);
  }
  if (alerts.length) {
    const ins = db.prepare(
      "INSERT INTO alerts (ts, source, product_id, product_name, rule_id, kind, message, group_id, group_label) VALUES (?,?,?,?,?,?,?,?,?)"
    );
    for (const a of alerts) {
      ins.run(a.ts, a.source, a.productId, a.productName, a.ruleId, a.kind, a.message, a.groupId, a.groupLabel);
    }
  }
  return alerts;
}
...
export function runWatch(db,watchCfg){
  db.exec('SAVEPOINT watch_run');
  try{const result=runWatchInner(db,watchCfg);db.exec('RELEASE watch_run');return result;}
  catch(error){db.exec('ROLLBACK TO watch_run');db.exec('RELEASE watch_run');throw error;}
}
