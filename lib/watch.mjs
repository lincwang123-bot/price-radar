...
function evalRule(db, rule, sourceId, snapList, snapById) {
  ...
  for (const prod of products) {
    ...
    const offers = offersOfProduct(db, sourceId, latest.snapshot_id, prod.product_id);
    const curLow = prod.lowest_price;
    const curCheapest = cheapestOffer(offers);
    const prevProd = prevProducts.get(prod.product_id) ?? null;
    const prevLow = prevProd?.lowest_price ?? null;
    const prevCheapest = cheapestOffer(prevOffersByProduct.get(prod.product_id) ?? []);
    ...
    if (rule.kind === "drop_pct" && curLow != null && rule.pct != null) {
      ...
      const snaps = recentSnapshots(db, sourceId, n + 1); // 含当前
      const windowLows = [];
      for (let i = 1; i < snaps.length; i++) {
        const row = db.prepare(
          "SELECT lowest_price FROM products WHERE source=? AND snapshot_id=? AND product_id=?"
        ).get(sourceId, snaps[i].snapshot_id, prod.product_id);
        if (row?.lowest_price != null) windowLows.push(row.lowest_price);
      }
...
export function runWatch(db, watchCfg) {
  ...
  for (const [sourceId, sourceRules] of bySource) {
    const snaps = recentSnapshots(db, sourceId, 2);
    if (!snaps.length) continue;
    const latestId = snaps[0].snapshot_id;
