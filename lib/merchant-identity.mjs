const blocked = new Set(['cardnav.xyz','goaihop.com','relaywatch.online']);
export function merchantIdentityForUrl(value) {
  try {
    ...
    if (['wzyp.cn','www.wzyp.cn','ldxp.cn','www.ldxp.cn','pay.ldxp.cn'].includes(host)) {
      const match = url.pathname.match(/^\/(?:shop\/)?([a-zA-Z0-9_-]{1,100})\/?$/);
      return match && !/^(?:shop|goods|item|product|products|login|register|admin|api|shopApi|account|order|orders|user|search|category|about|help|favicon|robots|sitemap)$/i.test(match[1]) ? `shop:${host==='pay.ldxp.cn'?'ldxp-pay':host.includes('wzyp')?'wzyp':'ldxp'}:${match[1]}` : null;
    }
...
export function merchantIdentityForOffer(offer = {}) {
  try {
    ...
    const host = url.hostname.toLowerCase();
    if (['16688.com.cn','www.16688.com.cn','wzyp.cn','www.wzyp.cn','ldxp.cn','www.ldxp.cn','pay.ldxp.cn'].includes(host)) {
