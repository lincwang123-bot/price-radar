...
import { offerProvenance } from './offer-provenance.mjs';
import { deliveryEvidence } from './delivery-evidence.mjs';

function snapshotExtra(value, sourceName) {
  let extra = value;
  if (typeof extra === 'string') { try { extra = JSON.parse(extra); } catch { extra = {}; } }
  if (!extra || typeof extra !== 'object' || Array.isArray(extra)) extra = {};
  return { ...extra, ...(Object.hasOwn(extra, 'deliveryEvidence') ? { deliveryEvidence:deliveryEvidence(extra.deliveryEvidence) } : {}), upstreamSourceName:sourceName ?? null };
}
...
export function openDb(dbPath) {
  ...
  for(const column of ['source_type','source_url','last_updated_at','last_verified_at','merchant_id','recorded_at']) {
    if(!columns.has(column))db.exec(`ALTER TABLE offers ADD COLUMN ${column} TEXT`);
  }
  db.exec('CREATE INDEX IF NOT EXISTS idx_offers_identity_history ON offers(source,product_id,offer_id)');
  db.exec('CREATE INDEX IF NOT EXISTS idx_offers_identity_recorded ON offers(source,product_id,offer_id,recorded_at DESC)');
  return db;
}
...
export function storeSnapshot(db, snapshot) {
  ...
  const observation={...snapshot,fetchedAt:snapshot.fetchedAt??new Date().toISOString()};
  const exists = db
  ...
  try {
    ...
    const insOffer = db.prepare(
      `INSERT INTO offers
         (source, snapshot_id, product_id, offer_id, source_id, source_name, store_name,
          ...
          source_type,source_url,last_updated_at,last_verified_at,merchant_id,recorded_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    );
    const previousOffer=db.prepare('SELECT * FROM offers WHERE source=? AND product_id=? AND offer_id=? AND recorded_at<=? ORDER BY recorded_at DESC,rowid DESC LIMIT 1');
    const previousLegacyOffer=db.prepare('SELECT o.* FROM offers o JOIN snapshots s ON s.source=o.source AND s.snapshot_id=o.snapshot_id WHERE o.source=? AND o.product_id=? AND o.offer_id=? AND o.recorded_at IS NULL AND s.fetched_at<=? ORDER BY s.fetched_at DESC,s.rowid DESC LIMIT 1');
    ...
    for (const p of products) {
      ...
      for (const o of offers) {
        const previous=previousOffer.get(source,p.productId,o.offerId,observation.fetchedAt)||previousLegacyOffer.get(source,p.productId,o.offerId,observation.fetchedAt);
        const provenance=offerProvenance(source,o,observation,previous);
        insOffer.run(
          source, snapshotId, p.productId, o.offerId,
          o.sourceId ?? null, provenance.source_name, o.storeName ?? o.sourceName ?? null,
          o.title ?? null, o.price ?? null, o.currency ?? null,
          o.status ?? null, o.stockCount ?? null, o.url ?? null,
          o.capturedAt ?? null, o.expiresAt ?? null,
          JSON.stringify(snapshotExtra(o.extra,o.sourceName)),
          provenance.source_type,provenance.source_url,provenance.last_updated_at,provenance.last_verified_at,provenance.merchant_id,provenance.recorded_at
        );
      }
