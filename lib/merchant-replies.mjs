...
export function recordMerchantReply(db,input,{now=new Date(),actor='codex:verified-gmail'}={}){
 ...
 try{
  ...
  const prior=db.prepare('SELECT * FROM merchant_reply_rounds WHERE message_id=? OR (application_id=? AND fingerprint=?) ORDER BY id LIMIT 1').get(messageId,id,fingerprint);
  if(prior){
