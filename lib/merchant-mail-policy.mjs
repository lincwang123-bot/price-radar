// One receipt + one outcome per merchant application, including its original supply submission.
...
function reconcileGroup(db,group){
 const rows=db.prepare(`SELECT id,stage,status,attempts,recipient FROM merchant_mail_outbox WHERE application_id IN (${group.ids.map(()=>'?').join(',')}) AND stage<>'reply' ORDER BY id`).all(...group.ids);
 ...
 const spent=rows.filter(attempted),keep=new Set();
 let remaining=Math.max(0,2-spent.length);
 for(const slot of ['receipt','outcome']){
  const used=spent.filter(row=>kind(row)===slot);
  if(used.length){
   // Only the first event in a slot can continue a safe connection retry.
   if(spent.indexOf(used[0])<2&&eligible(used[0]))keep.add(used[0].id);
  }else if(remaining){
   const candidates=rows.filter(row=>pending.has(row.status)&&eligible(row)&&kind(row)===slot&&!attempted(row));
 ...
 for(const row of rows){
  if(pending.has(row.status)&&!keep.has(row.id)){
   const code=!eligible(row)?'mail_policy_stage':attempted(row)||spent.some(other=>kind(other)===kind(row))||spent.length>=2?'mail_policy_limit':'mail_policy_replaced';
