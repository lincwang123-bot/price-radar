...
function reconcileGroup(db,group){
 ...
 const app=(tableExists(db,'merchant_applications')?db.prepare('SELECT status,email FROM merchant_applications WHERE public_id=?').get(group.key):null)
  ||(db.prepare('PRAGMA table_info(merchant_supply_intake)').all().some(row=>row.name==='status')?db.prepare('SELECT status,email FROM merchant_supply_intake WHERE source_submission_id=?').get(group.key):null);
