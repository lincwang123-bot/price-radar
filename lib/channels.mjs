...
export function offerChannel(offer){
  let host;try{const u=new URL(offer.url);if(!["https:","http:"].includes(u.protocol)||u.username||u.password)throw new Error();host=u.hostname.replace(/\.$/, '').replace(/^www\./, '');}catch{return{id:"unknown",label:"未确认渠道",framework:null};}
  if(host==="16688.com.cn")return{id:"16688",label:"16688",framework:null};
  if(["wzyp.cn","ldxp.cn","pay.ldxp.cn"].includes(host))return{id:"ldxp",label:"链动小铺",framework:null};
