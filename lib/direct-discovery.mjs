...
export async function discover16688({dataDir,fetchImpl=globalThis.fetch,sleep=delay,now=Date.now(),refresh=false}={}) {
  ...
  try {
    ...
    const goods=[],seen=new Set(),priority=new Map(),retired=new Set();let total=null;
    for(let page=1;page<=MAX_PAGES;page++) {
      ...
      for(const row of data.list){if(!goodsNumber(row?.goods_no)||seen.has(row.goods_no))invalid();seen.add(row.goods_no);goods.push(row.goods_no);
        if(retiredCatalogItem({title:row.name}))retired.add(row.goods_no);
        priority.set(row.goods_no,/(?:gpt|codex|claude|gemini|grok|cursor|perplexity|notion|manus|api|邮箱)/i.test(String(row.name||''))?1:0);}
    ...
    const candidates=goods.filter(id=>!knownSeeds.has(id)&&!retired.has(id)).sort((a,b)=>(Date.parse(state.checkedGoods[a])||0)-(Date.parse(state.checkedGoods[b])||0)||(priority.get(b)-priority.get(a)));
    let inspected=0;
