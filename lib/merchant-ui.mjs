...
import { preflightGuidance, listPreflightGuidance } from './merchant-preflight-guidance.mjs';
...
.merchant .guidance-library{margin-top:22px;border-top:1px solid #e5ebe5;padding-top:14px}.merchant .guidance-library summary{cursor:pointer;padding:10px 0;min-height:44px}.merchant .guidance-library details{padding:8px 0;border-top:1px solid #e5ebe5}.merchant .merchant-copy-text{line-height:1.7;min-height:190px}.merchant .guidance-next{padding:12px 14px;background:#f2f6f3;border-radius:8px}.merchant .copy-block{margin-top:20px}.merchant .copy-block label{margin-bottom:8px}.merchant .copy-block .actions{margin-top:12px;align-items:center}.merchant .copy-block .actions .muted{flex:1;min-width:180px}
</style>`;
...
function preflightContent(app, preflight, csrfToken) {
  ...
  const library = listPreflightGuidance(app);
  const diagnostic = `<div class="preflight-guidance"><h3>${esc(guidance.title)}</h3><p>${esc(guidance.explanation)}</p>${guidance.nextStep?`<p class="guidance-next"><strong>下一步：</strong>${esc(guidance.nextStep)}</p>`:''}${guidance.merchantMessage?`<div class="copy-block">${copyMessageContent(guidance.merchantMessage,'preflight-current-message')}</div>`:''}<details class="guidance-library"><summary>其他情况的沟通文案</summary><p class="muted">这些是按情况准备的模板，不代表当前店铺已经发生这些问题。请选择符合实际情况的文案。</p>${library.map((item,index)=>`<details><summary>${esc(item.title)}</summary><p>${esc(item.explanation)}</p><p><strong>下一步：</strong>${esc(item.nextStep)}</p><div class="copy-block">${copyMessageContent(item.merchantMessage,'preflight-library-'+index)}</div></details>`).join('')}</details></div>`;
  ...
  return `<section class="panel"><h2>接入测试</h2><p><span class="badge">${esc(labels[state] || '尚未测试')}</span></p><p>${esc(help[state] || '系统自动排队检测公开目录，通常在下一轮采集中完成；无需手动点击。')}</p>${result ? `${countSummary}<p class="muted">测试时间：${esc(formatTime(result.checkedAt))}</p>${samples.length ? `<h3>商品样例</h3>${samples.map(sample=>`<div class="audit"><strong>${esc(sample.title)}</strong><p>${esc(sample.currency)} ${esc(sample.price)} · ${shopLink(sample.url, '打开商品页')}</p></div>`).join('')}` : ''}` : ''}${diagnostic}<p class="muted">测试只读取公开目录，不自动发布报价，也不验证真实交易。0 条有效报价不能视为接入成功。</p><div class="actions"><a class="secondary" href="/admin/merchants/${encodeURIComponent(app.id)}">刷新结果</a></div></section>
<form action="/admin/merchants/${encodeURIComponent(app.id)}" method="post" id="merchant-preflight"><h2>手动重测（仅在资料修正后使用）</h2><input type="hidden" name="csrf" value="${esc(csrfToken)}"><input type="hidden" name="version" value="${esc(app.version)}"><p class="muted">自动检测不等于审核通过。重新测试仅读取公开目录。</p><div class="actions"><button name="action" value="auto_test" type="submit">${preflight ? '重新测试接入' : '测试接入'}</button></div></form>`;
}
