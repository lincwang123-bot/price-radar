...
${items.length?items.map(app=>`<article class="panel"><div class="row"><div><h2><a href="/admin/merchants/${encodeURIComponent(app.id)}">${esc(app.shopName)}</a></h2><p>${shopLink(app.shopUrl)}</p></div><div><span class="badge">${esc(statuses[app.status]??app.status)}</span> <span class="badge">${esc(app.status==='pending'?preflightSummary(preflights[app.id]):collectionLabel(app,health[app.id]??health))}</span></div></div><p class="muted">${app.email?esc(mailSummary(mails[app.id])):'待补邮箱'} · ${esc(app.id)} · ${esc(platforms[app.platform]??app.platform)} · ${esc(formatTime(app.createdAt))}</p></article>`).join(''):'<div class="panel"><h2>暂无申请</h2><p class="muted">此状态下还没有店铺申请。</p></div>'}
