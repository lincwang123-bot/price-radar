...
export function reviewMerchantApplication(db, id, review, { bridgeDir = merchantBridgeDir(db), now = new Date() } = {}) {
  if (!review || !['approve', 'reject', 'pause', 'request_info'].includes(review.action)) fail(422, '审核操作无效');
  ...
  try {
    ...
    queueMerchantMail(db,{id,email:row.email,stage:review.action==='request_info'?'need_info':status,eventKey:id+':review:'+(row.version+1),now});
    if (bridgeDir) {
