...
function validatedMerchantPayload(payload) {
  ...
  return { shopName, shop, productAreas, contact, details };
}
function insertMerchantApplication(db, values, time, client = null) {
  const { shopName, shop, productAreas, contact, details } = values, timestamp = time.toISOString();
...
export function conversionFormValues(payload) {
  const values = Object.fromEntries(['shopName','shopUrl','contact','details','note'].map(key => {
