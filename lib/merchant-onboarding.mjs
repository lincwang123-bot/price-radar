...
function application(row) {
  ...
  return { id: row.public_id, createdAt: row.created_at, updatedAt: row.updated_at, shopName: row.shop_name,
    ...
    contact: row.contact, email: row.email || null, details: row.details, consentAt: row.consent_at, status: row.status,
...
function insertMerchantApplication(db, values, time, client = null) {
  ...
  db.prepare(`INSERT INTO merchant_applications(public_id,created_at,updated_at,shop_name,shop_url,identity,platform,product_areas,contact,details,consent_at,client_hash,email)
    VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(id, timestamp, timestamp, shopName, shop.shopUrl, shop.identity, shop.platform, JSON.stringify(productAreas), contact, details, timestamp, client, email);
...
export function createMerchantApplication(db, payload, { clientAddress, now = new Date(), secret = PROCESS_SECRET } = {}) {
  const values = validatedMerchantPayload(payload);
  if (!values.email) fail(422,'请填写通知邮箱，用于接收接入测试及审核进度');
...
export function reviewMerchantApplication(db, id, review, { bridgeDir = merchantBridgeDir(db), now = new Date() } = {}) {
  if (!review || !['approve', 'reject', 'pause', 'request_info','reply'].includes(review.action)) fail(422, '审核操作无效');
  const note = clean(review.note, '核验说明', 1500, review.action==='reply'?0:5);
  ...
  try {
    ...
    const status = { approve: 'approved', reject: 'rejected', pause: 'paused', request_info:row.status }[review.action];
    ...
    queueMerchantMail(db,{id,email:row.email,stage:review.action==='request_info'?'need_info':status,eventKey:id+':review:'+(row.version+1),publicReply,now});
    if (bridgeDir) {
...
export function updateMerchantEmail(db,id,email,{actor='admin',expectedVersion,confirmed=false,now=new Date()}={}) {
  ...
  try {
    ...
    queueMerchantMail(db,{id,email:address,stage:app.status==='pending'?'received':app.status,eventKey:id+':email:'+action.lastInsertRowid,now});
    db.exec('COMMIT');
  }catch(error){if(db.isTransaction)db.exec('ROLLBACK');throw error;}
}
