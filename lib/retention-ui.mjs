...
export function followingContent(){return `<section class="retention-page" data-following-page>
 ...
 <section class="retention-account" aria-label="关注同步"><div><strong data-account-title>本机关注清单</strong><p data-account-copy>保存在当前浏览器。确认邮箱后可跨设备同步，并接收自选提醒。</p></div><div class="retention-actions" data-account-actions><button class="retention-button" data-login>确认邮箱并同步</button></div></section>
 ...
 <p class="retention-muted" data-list-summary>正在读取关注清单…</p><div class="retention-grid" data-watch-list></div>
...
export function retentionDialogs(){return `<div class="retention-toast" data-retention-toast role="status" aria-live="polite" hidden></div>
 ...
 <dialog class="retention-dialog" id="retention-login-dialog"><div class="retention-dialog-head"><h2>用邮箱同步关注</h2><button class="retention-close" data-close-dialog aria-label="关闭邮箱确认">×</button></div>
 <form id="retention-email-form"><label>邮箱<input type="email" name="email" autocomplete="email" required maxlength="254" placeholder="you@example.com"></label><label class="retention-checkbox"><input type="checkbox" name="consent" required> 我同意用此邮箱验证身份、同步关注，并接收自己选择的提醒；可随时暂停或退订。</label><p class="retention-form-error" role="alert" data-form-error></p><button class="retention-button primary" type="submit">获取验证码</button></form>
 <form id="retention-code-form" hidden><p data-code-copy>请查收 6 位验证码，10 分钟内有效。</p><label>验证码<input name="code" inputmode="numeric" autocomplete="one-time-code" pattern="[0-9]{6}" maxlength="6" required></label><p class="retention-form-error" role="alert" data-form-error></p><button class="retention-button primary" type="submit">确认并同步</button><button class="retention-button" type="button" data-retry-code>重新获取</button></form></dialog>`;}
