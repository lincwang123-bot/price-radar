...
function feedbackPayload(payload) {
  ...
  if(['sponsor_apply','merchant_claim'].includes(topic)){
    ...
    if(topic==='sponsor_apply'){
      ...
      if(quote){
        if(metadata.rateVersion!==quote.version)throw new SubmissionError(422,'赞助报价已更新，请刷新页面并确认当前价格后重新提交。');
        ...
        fields['参考费用']='¥'+quote.amount+' / '+quote.days+'天 / 每家';fields['价目版本']=quote.version;fields['说明']='共享位置最多4家；提交不扣款、不锁位，档期人工确认';
      }
    }
