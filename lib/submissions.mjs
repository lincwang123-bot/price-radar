import {sponsorQuote} from './sponsor-plans.mjs';
import { DatabaseSync } from "node:sqlite";
import { createHmac, randomBytes } from "node:crypto";
import { mkdirSync } from "node:fs";
import path from "node:path";
import { initMerchantSchema } from './merchant-onboarding.mjs';
...
const FEEDBACK_TOPICS = new Set([
  ...
  "sponsor_apply", "merchant_claim",
]);
const COOPERATION_TOPICS = new Set(["supply", "demand"]);
const PRODUCT_AREAS = new Set(["chatgpt", "claude", "gemini", "grok_x", "mail_verify", "other"]);
...
export function openSubmissionsDb(dbPath, { now = new Date() } = {}) {
  ...
  try {
    ...
    for (const table of ['feedback_submissions','cooperation_submissions']) {
      if (!db.prepare(`PRAGMA table_info(${table})`).all().some(row=>row.name==='email')) db.exec(`ALTER TABLE ${table} ADD COLUMN email TEXT`);
    }
    if (!db.prepare('PRAGMA table_info(submission_actions)').all().some(row=>row.name==='public_reply')) db.exec("ALTER TABLE submission_actions ADD COLUMN public_reply TEXT NOT NULL DEFAULT ''");
...
function feedbackPayload(payload) {
  ...
  if(['sponsor_apply','merchant_claim'].includes(topic)){
    const subject=text(payload.subject,{field:'商家或产品',min:2,max:120,required:true});
    const contextUrl=safeContextUrl(payload.contextUrl,{httpsOnly:true});
    if(!contextUrl)throw new SubmissionError(422,'请填写店铺或公开介绍链接');
    const contact=text(payload.contact,{field:'联系方式',min:3,max:128,required:true});
    const explanation=text(payload.details,{field:'申请说明',min:10,max:1000,required:true});
    if(payload.consent!==true)throw new SubmissionError(422,'请确认申请信息真实且不含敏感凭据');
    const metadata=payload.metadata;
    if(!metadata||typeof metadata!=='object'||Array.isArray(metadata))throw new SubmissionError(422,'请补全申请选项');
    let fields;
    if(topic==='sponsor_apply'){
      fields={广告位置:oneOf(metadata.placement,new Set(['product','category','home','site']),'广告位置'),投放时长:oneOf(metadata.duration,new Set(['7d','14d','28d','1m','3m','discuss']),'投放时长')};
      const quote=sponsorQuote(metadata.placement,metadata.duration);
      if(['7d','14d','28d'].includes(metadata.duration)&&!quote)throw new SubmissionError(422,'该位置暂无标准档期');
      if(quote){
        if(metadata.rateVersion!==quote.version)throw new SubmissionError(422,'合作方案已更新，请刷新页面后重新提交。');
        let target;try{target=new URL(String(metadata.targetPage||''),'https://airadar.vip');if(typeof metadata.targetPage!=='string'||metadata.targetPage.length>1000||!metadata.targetPage||target.origin!=='https://airadar.vip'||target.pathname!=='/'||[...target.searchParams.keys()].some(k=>!['family','product','delivery','channel','spec','currency','sort','page'].includes(k)))throw Error();if(metadata.placement==='home'&&target.search||metadata.placement==='category'&&(!target.searchParams.get('family')||target.searchParams.has('product'))||metadata.placement==='product'&&(!target.searchParams.get('family')||!target.searchParams.get('product')))throw Error();}catch{throw new SubmissionError(422,'请填写与所选位置对应的本站首页、分类页或产品页链接');}
        fields['目标页面']=target.pathname+target.search;
        fields['合作费用']='待沟通，尚未报价';fields['说明']='共享位置最多4家；提交不扣款、不锁位，档期人工确认';
      }
    }
    else fields={申请身份:oneOf(metadata.relationship,new Set(['owner','employee','authorized']),'申请身份'),核验方式:oneOf(metadata.verification,new Set(['domain','store_notice','manual']),'身份核验方式')};
    rejectSecrets([subject,contextUrl,contact,explanation]);
    return {topic,subject,contextUrl,contact,details:Object.entries(fields).map(([k,v])=>k+'：'+v).join('\n')+'\n\n'+explanation};
  }
...
export function createSubmission(db, payload, {
  ...
  const email = payload.email ? normalizeEmail(payload.email) : normalizeEmail(normalized.contact);
  if (payload.email && !email) throw new SubmissionError(422,'请填写一个有效的通知邮箱');
  ...
  if (email && db.prepare("SELECT COUNT(*) n FROM merchant_mail_outbox WHERE recipient=? COLLATE NOCASE AND stage IN ('received','submission_received') AND created_at>=?").get(email,new Date(now.getTime()-86400000).toISOString()).n>=5) throw new SubmissionError(429,'此邮箱今天提交过于频繁，请稍后再试');
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    ...
    try {
      ...
      if(email)db.prepare(`UPDATE ${table} SET email=? WHERE public_id=?`).run(email,id);
      queueMerchantMail(db,{id,email,stage:'submission_received',eventKey:id+':submitted',now});
      db.exec('COMMIT');
...
export function updateSubmissionStatus(db, publicIdValue, status, { actor = "ssh-cli", note = "", publicReply = "", now = new Date() } = {}) {
  ...
  const reply = text(publicReply, {field:'发给用户的回复',max:1500}) || '';
  rejectSecrets([reply]);
  const safeActor = text(actor, { field: "操作者", max: 100, required: true });
  db.exec("BEGIN IMMEDIATE");
  try {
    const old = db.prepare(`SELECT status,email,contact FROM ${table} WHERE public_id = ?`).get(id);
    if (!old) { db.exec("ROLLBACK"); return false; }
    const hasIntake=id.startsWith('CO-')&&db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='merchant_supply_intake'").get();
    const intakeEmail=hasIntake?db.prepare('SELECT email FROM merchant_supply_intake WHERE source_submission_id=?').get(id)?.email:null;
    const email=normalizeEmail(intakeEmail)||normalizeEmail(old.email)||normalizeEmail(old.contact);
    if(reply&&!email)throw new SubmissionError(422,'该投稿未留有效邮箱，不能发送回复；请先通过原联系渠道核对');
    if(reply&&old.status===status&&db.prepare('SELECT id FROM submission_actions WHERE public_id=? AND status=? AND public_reply=? AND created_at>=?').get(id,status,reply,new Date(+now-300000).toISOString())) {db.exec('COMMIT');return true;}
    db.prepare(`UPDATE ${table} SET status = ? WHERE public_id = ?`).run(status, id);
    const action=db.prepare("INSERT INTO submission_actions(public_id,created_at,actor,previous_status,status,note,public_reply) VALUES(?,?,?,?,?,?,?)")
      .run(id, now.toISOString(), safeActor, old.status, status, safeNote,reply);
    const stage=old.status!==status?({reviewing:'submission_reviewing',resolved:'submission_resolved',accepted:'submission_accepted',closed:'submission_closed',rejected:'submission_rejected'}[status]):null;
    if(stage||reply)queueMerchantMail(db,{id,email,stage:stage||'reply',publicReply:reply,eventKey:id+':action:'+action.lastInsertRowid,now});
