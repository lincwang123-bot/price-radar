...
function feedbackPayload(payload) {
  ...
  if(['sponsor_apply','merchant_claim'].includes(topic)){
    ...
    let fields;
    if(topic==='sponsor_apply')fields={广告位置:oneOf(metadata.placement,new Set(['product','category','home','site']),'广告位置'),投放时长:oneOf(metadata.duration,new Set(['1m','3m','discuss']),'投放时长')};
