...
export const DIRECTORY_DELIVERIES = Object.freeze([
  {kind:'recharge',label:'代充'},{kind:'account',label:'成品号'},{kind:'shared',label:'共享或合租'},
  {kind:'seat',label:'席位'},{kind:'code',label:'卡密（用途待确认）'},{kind:'unknown',label:'交付待确认'},
]);
const deliveryEnabled = definition => !['mail','other'].includes(definition.category) && !/credits|service|claim-link/.test(definition.key);

export const DIRECTORY_CATEGORIES = Object.freeze([
  ['chatgpt', 'ChatGPT'], ['claude', 'Claude'], ['gemini', 'Gemini'],
  ['grok', 'Grok'], ['x', 'X'], ['mail', '邮箱'],
  ['suno', 'Suno'], ['cursor', 'Cursor'], ['perplexity', 'Perplexity'],
  ['notion', 'Notion AI'], ['manus', 'Manus'], ['microsoft', 'Microsoft'], ['other', '其他'],
].map(([key, label], index) => Object.freeze({ key, label, primary: index < 6 })));
...
const definitions = [
  ...
  ...[['gmail-account', 'Gmail / Google 邮箱'], ['icloud-email', 'iCloud 邮箱'], ['outlook-account', 'Outlook / Hotmail 邮箱'], ['email-account', '其他邮箱'], ['education-email', '教育邮箱']].map(([key, name]) => ['mail', key, name]),
  ['other', 'identity-verification', '真人 / KYC 验证'],
...
function definitionFor(list, product, offer) {
  if (retiredCatalogSource(list.source) || retiredCatalogItem(product) || retiredCatalogItem(offer)) return undefined;
  ...
  const trusted = ['direct-shops', 'cardnav-official'].includes(list.source);
  ...
  if (title) {
    const classified = classifyDirectOffer({ title, sourceId: offer.source_id || offer.sourceId, extra:list.source==='direct-shops'?offer.extra:undefined });
...
function quoteDefinition(list, product, offer) {
  ...
  const canonical = ['direct-shops'].includes(list.source) ? byId.get(product.product_id) : null;
  const title = String(offer.title || '').trim();
  ...
  // it again would discard valid aliases and their actual delivery wording.
  ...
  const explicit = classifyDirectOffer({ title: title.replace(/已接码|已验证/g, ''), sourceId: offer.source_id || offer.sourceId, extra:list.source==='direct-shops'?offer.extra:undefined });
...
// Share the exact directory identity with opt-in watchlists; search buckets are
// never treated as product identities by the notification layer.
export const directoryQuoteIdentity=(list,product,offer)=>quoteDefinition(list,product,offer)||null;

function quoteEligible(list, product, offer) {
  if (retiredCatalogSource(list.source) || retiredCatalogItem(product) || retiredCatalogItem(offer)) return false;
  const reference = ['cardnav-official'].includes(list.source);
...
export function directoryQuotes(product, { sort = 'price_asc', channel = 'all', spec = '', currency = '', delivery = 'all', merchantBadges = [], xProfiles = [] } = {}) {
  ...
  const all = [...deduped.values()].map(entry=>({...entry,delivery:offerDelivery(entry.offer),showDelivery:!!product?.deliveryEnabled}));
  const deliveries = DIRECTORY_DELIVERIES.map(group=>({...group,count:all.filter(entry=>!entry.reference && entry.delivery.kind===group.kind && (channel==='all'||entry.channel?.id===channel) && (!currency||(entry.offer.currency||entry.product.currency)===currency)).length})).filter(group=>group.count);
  const scope = product?.deliveryEnabled && delivery !== 'all' ? all.filter(entry=>entry.reference || entry.delivery.kind===delivery) : all;
  const specMap = new Map();
  for (const entry of scope) {
    const key = specKey(entry);
    if (!key) continue;
    if (!specMap.has(key)) specMap.set(key, { key, label: entry.offer.comparison_label?.trim() || (entry.reference ? '参考报价 · 规格待确认' : '规格待确认'), count: 0 });
    specMap.get(key).count++;
  }
  const entries = scope.filter(entry => (channel === 'all' || entry.channel.id === channel) && (!spec || specKey(entry) === spec) && (!currency || (entry.offer.currency || entry.product.currency) === currency));
  // Only the explicitly selected comprehensive view uses reviewed private state.
  // Keep price modes, catalogue minimums and collection previews price-only.
  const priorities = new Map();
  if (sort === 'comprehensive') {
    const listed = new Set(merchantBadges.filter(row => row.identityVerifiedAt).map(row => row.identity));
    const linked = new Set(xProfiles.map(row => row.identity));
    for (const entry of entries) {
      const identity = entry.reference ? null : merchantIdentityForOffer({...entry.offer, source: entry.list.source});
      priorities.set(entry, identity ? linked.has(identity) ? 2 : listed.has(identity) ? 1 : 0 : 0);
    }
  }
  entries.sort((a, b) => currencyOrder(a.offer.currency || a.product.currency, b.offer.currency || b.product.currency)
    || (sort === 'comprehensive' ? priorities.get(b) - priorities.get(a) : 0)
  ...
  return { entries, shopCount: new Set(merchantEntries.map(shopKey).filter(Boolean)).size,
    ...
    deliveries, specs: [...specMap.values()], currencies: [...new Set(scope.map(entry => entry.offer.currency || entry.product.currency))].sort(currencyOrder) };
}
...
export function buildProductDirectory(lists = []) {
  ...
  for (const list of lists.filter(Boolean)) for (const product of list.products || []) {
    ...
    for (const offer of offers) {
      ...
      if (quoteEligible(list, product, offer)) {
        ...
        (group.quoteEntries ||= []).push({ list, product, offer, channel: offerChannel(offer), reference: ['cardnav-official'].includes(list.source) });
      }
      if (definitionFor(list, product, offer)?.key !== definition.key) continue;
      if (!eligible(list, product, offer)) continue;
      products.get(definition.key).entries.push({ list, product, offer, channel: offerChannel(offer), reference: ['cardnav-official'].includes(list.source) });
    }
  }
  for (const { definition, entries, references, quoteEntries = [] } of [...products.values()].sort((a, b) => a.definition.rank - b.definition.rank || a.definition.name.localeCompare(b.definition.name))) {
    ...
    categories.find(category => category.key === definition.category).products.push({ key: definition.key, name: definition.name, deliveryEnabled:deliveryEnabled(definition), entries, quoteEntries, variants, references: references.filter(entry => entry.list.source !== 'ldxp-goods') });
  }
  return categories;
}
