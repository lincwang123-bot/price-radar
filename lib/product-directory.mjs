...
function quoteDefinition(list, product, offer) {
  ...
  const brandProducts = [
    /(?:chat\s*gpt|\bgpt\b|openai)\s*[- ]*(?:plus|pro|go|team|business|free)/i,
...
export function directoryQuotes(product, { sort = 'price_asc', channel = 'all', spec = '', currency = '' } = {}) {
  ...
  for (const entry of product?.quoteEntries || product?.entries || []) {
    ...
    const key = JSON.stringify([!!entry.reference, destinationKey(entry.offer.url), knownSpec || String(entry.offer.title || ''), entry.offer.currency || entry.product.currency]);
