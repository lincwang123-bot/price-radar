...
export async function probeMerchantCatalog(merchant, ctx, capturedAt, deadline) {
  ...
  const fetchImpl = (ctx.merchantFetchFactory ?? createPublicNetworkFetch)(target.origin,{totalTimeoutMs:Math.max(1,Math.min(30000,deadline-Date.now())),
    maxRequests:target.kind==='bbshare'?BBSHARE_MAX_REQUESTS:PUBLIC_HTML_MAX_REQUESTS});
  const options = {capturedAt,fetchImpl,timeoutMs:8000,maxBytes:1024*1024,maxRedirects:0,requestDelayMs:0,deadline,sleep:ctx.sleep};
...
export async function collectApprovedMerchants(ctx, {manifest,existingOffers=[],capturedAt,maxCacheAgeMinutes=1440} = {}) {
  ...
  for (const {merchant,file,cached} of states) {
    ...
    health.push({id:merchant.id,identity:merchant.identity,name:merchant.shopName,status,checkedAt:state.checkedAt,lastSuccess:state.lastSuccess,offerCount:usableOffers.length,validCount,
