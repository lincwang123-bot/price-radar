...
import { collectPublicHtml, PUBLIC_HTML_MAX_REQUESTS } from '../collectors/direct/public-html.mjs';
...
export async function probeMerchantCatalog(merchant, ctx, capturedAt, deadline) {
  ...
  const fetchImpl = retryingPublicFetch((ctx.merchantFetchFactory ?? createPublicNetworkFetch)(target.origin,{totalTimeoutMs:Math.max(1,Math.min(30000,deadline-Date.now())),
    maxRequests:target.kind==='bbshare'?BBSHARE_MAX_REQUESTS:target.kind==='dujiaokaHtml'?DUJIAOKA_HTML_MAX_REQUESTS:PUBLIC_HTML_MAX_REQUESTS}),{deadline,sleep:ctx.sleep,maxRetries:1});
  ...
  if (!target.kind) {
    // Existing API recognition stays first. An unrecognized directory can also
    // use explicitly linked public HTML; access denials always stop the attempt.
