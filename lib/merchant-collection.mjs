...
export async function probeMerchantCatalog(merchant, ctx, capturedAt, deadline) {
  ...
  const fetchImpl = retryingPublicFetch((ctx.merchantFetchFactory ?? createPublicNetworkFetch)(target.origin,{totalTimeoutMs:Math.max(1,Math.min(30000,deadline-Date.now())),
    ...(target.kind==='bbshare'?{maxRequests:BBSHARE_MAX_REQUESTS}:target.kind==='dujiaokaHtml'?{maxRequests:DUJIAOKA_HTML_MAX_REQUESTS}:{})}),{deadline,sleep:ctx.sleep,maxRetries:1});
  const options = {capturedAt,fetchImpl,timeoutMs:8000,maxBytes:1024*1024,maxRedirects:0,requestDelayMs:target.kind==='shopApi'?1000:0};
  ...
  if (!target.kind) {
    ...
    for (const [kind,endpoint] of [['dujiao','/api/v1/public/products?page=1&page_size=100'],['kami','/user/api/index/commodity?limit=100&page=1']]) {
      ...
      try {
        ...
        probeFailures.push(error.status === 404 ? 'not_found' : 'invalid_catalog');
        continue;
      }
    ...
    if (!target.kind) return {unsupported:true,
      ...(probeFailures.length === 2 && probeFailures.every(reason => reason === 'not_found')
        ? {reasonCode:'not_found',httpStatus:404}
        : probeFailures.includes('invalid_catalog') ? {reasonCode:'invalid_catalog'} : {reasonCode:'unsupported_platform'})};
  }
