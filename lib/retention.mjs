...
export function createRetention({db,submissionsDb,env=process.env,now=()=>new Date(),mailTransport,secret=env.RETENTION_SECRET||env.SUBMISSION_HASH_SECRET||randomBytes(32).toString('hex')}={}){
  ...
  if(!cached||force||+now()-last>60000){
   ...
   if(store){observeMarket(store.db,cached,now());
    ...
    for(const row of store.db.prepare('SELECT payload FROM retention_market').all()){
     const g=JSON.parse(row.payload);if(!known.has(g.id))cached.groups.push({...g,state:'unknown',price:null,maxPrice:null});
 ...
 const service={store,market,
  ...
  async route(req,res,url){
   if(['/assets/retention.js','/assets/retention.css'].includes(url.pathname)){
    if(!['GET','HEAD'].includes(req.method)){res.setHeader('Allow','GET, HEAD');json(res,405,{error:'不支持的请求方法'});return true;}
    const css=url.pathname.endsWith('.css');res.setHeader('Content-Type',css?'text/css; charset=utf-8':'text/javascript; charset=utf-8');res.end(req.method==='HEAD'?'':css?clientStyle:clientScript);return true;
   }
   ...
   try{
    ...
    if(action==='state'&&req.method==='GET'){
     ...
     json(res,200,{csrf,mailConfigured:mailConfiguration(env).configured,account:a?{email:a.email,emailEnabled:!!a.email_enabled}:null,watches:a?store.watches(a.id):[],notices:a?store.notices(a.id):[]});return true;
    }
    ...
    if(action==='request-code'){
     if(!mailConfiguration(env).configured)throw new RetentionError('邮件发送尚未配置，请先使用本机关注',503);
     if(input.consent!==true)throw new RetentionError('请确认邮箱验证和自选提醒用途');
     json(res,200,store.requestCode(input.email,client));return true;
    }
    if(action==='verify-code'){
     const result=store.verifyCode(input.requestId,input.code);cookies(res,req,'airadar_reader',result.token,30*86400,env);json(res,200,{ok:true});return true;
    }
    if(action==='event'){
    ...
    if(!a)throw new RetentionError('请先确认邮箱',401);
    ...
    if(action==='delete-account'){
     ...
     for(const table of ['retention_watches','retention_sessions','retention_notices','retention_mail'])store.db.prepare(`DELETE FROM ${table} WHERE account_id=?`).run(a.id);
  ...
  start(server){
   ...
   const tick=()=>{
    ...
    running=(async()=>{try{const m=market();store.observe(m);store.purge();await drainRetentionMail(store,m,{env,transport:mailTransport,now:now(),signal:controller.signal});}catch{console.error('[retention] 提醒处理未完成，下轮重试');}})().finally(()=>{running=null;});
   };
