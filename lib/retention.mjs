  ...
  async route(req,res,url){
   ...
   try{
    ...
    if(action==='delete-account'){
     ...
     for(const table of ['retention_watches','retention_sessions','retention_notices','retention_mail','reader_credentials'])store.db.prepare(`DELETE FROM ${table} WHERE account_id=?`).run(a.id);
