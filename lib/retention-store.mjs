...
export function createRetentionStore(db,{secret,now=()=>new Date()}={}){
 ...
 if(!db.prepare('PRAGMA table_info(retention_codes)').all().some(c=>c.name==='purpose'))db.exec("ALTER TABLE retention_codes ADD COLUMN purpose TEXT NOT NULL DEFAULT 'legacy'");
 ...
 const api={db,secret,digest,account,event,
  lastDigestAt(id){return db.prepare('SELECT value FROM retention_meta WHERE key=?').get('last_digest:'+id)?.value||null;},
  noteDigestAt(id,stamp=at()){db.prepare('INSERT INTO retention_meta VALUES(?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value').run('last_digest:'+id,stamp);},
  requestCode(value,client,{purpose='legacy',send=true}={}){
   ...
   db.prepare('INSERT INTO retention_codes(id,email,code_hash,created_at,expires_at,client_hash,purpose) VALUES(?,?,?,?,?,?,?)').run(id,email,digest(id+'|'+code),at(),new Date(+now()+10*60000).toISOString(),clientHash,purpose);
   if(send)queueMail('code:'+id,null,email,'code',{requestId:id,code,purpose});
   return {requestId:id};
  },
  verifyCode(id,code){
   const row=typeof id==='string'?db.prepare('SELECT * FROM retention_codes WHERE id=?').get(id):null;
   if(!row||row.purpose!=='legacy'||row.used||row.expires_at<=at()||row.attempts>=5)fail('验证码无效或已过期，请重新获取');
  ...
  observe(market){
   ...
   for(const a of db.prepare('SELECT * FROM retention_accounts WHERE email_enabled=1').all()){
    if(+now()-Date.parse(api.lastDigestAt(a.id)||'')<86400000)continue;
    if(db.prepare("SELECT 1 FROM retention_mail WHERE account_id=? AND kind='digest' AND status IN ('queued','retry','sending')").get(a.id))continue;
